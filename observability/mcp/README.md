...
Point `PROMETHEUS_URL` at the observability box, e.g. `http://metrics-muppet.example:9090`.
...
To enable the optional day-2 tools that invoke `keinctl`, set the binary,
context, and explicit mutation gate in the process environment. Leave the gate
unset to keep mutations disabled, which is the default.
...
is the operator's decision-support layer here: the advisory tools synthesize current
