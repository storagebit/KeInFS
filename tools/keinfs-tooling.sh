...
render_configs_command() {
  ...
  render_template "config/krs.toml.in" "${out_dir}/krs/default.toml" \
    ...
    KRS_LEASE_OWNER "$(toml_string "leasey-mcleaseface.example")" \
...
render_single_host_vm_lab_command() {
  ...
  local host_ip="" target_host="203.0.113.1" target_count="12" base_port="18080"
  local server_id="hosty-mchostface" rack_id="rackety-mcrackface" context_name="tiny-lab-boogaloo"
  ...
  local foundationdb_cluster_id
  foundationdb_cluster_id=${FOUNDATIONDB_CLUSTER_ID:-$(random_hex 16)}
  local foundationdb_cluster_contents="barnacle-bay:${foundationdb_cluster_id}@198.51.100.11:4500"
  local -a cp_nodes=(
    "quartermaster-quinn.example|198.51.100.11|203.0.113.11|$(quickstart_mac $((0x13)) $((0x11)))|$(quickstart_mac $((0x14)) $((0x11)))|kms-shard-0001|alloc-shard-00"
    "navigator-nell.example|198.51.100.12|203.0.113.12|$(quickstart_mac $((0x13)) $((0x12)))|$(quickstart_mac $((0x14)) $((0x12)))|kms-shard-0002|alloc-shard-01"
    "bosun-bubbles.example|198.51.100.13|203.0.113.13|$(quickstart_mac $((0x13)) $((0x13)))|$(quickstart_mac $((0x14)) $((0x13)))|kms-shard-0003|alloc-shard-02"
  )
  ...
  for ((index = 0; index < target_count; index++)); do
    ...
    printf -v target_id 'chunky-mcchunkface-%02d' "${index}"
  ...
  local kms_nats_url="nats://198.51.100.11:4222"
