...
argv=(
  ...
  --write-ingress-mode "${KST_WRITE_INGRESS_MODE:-interrupt}"
  ...
  --direct-read-mode "${KST_DIRECT_READ_MODE:-interrupt}"
  --direct-write-mode "${KST_DIRECT_WRITE_MODE:-interrupt}"
  --direct-read-workers "${KST_DIRECT_READ_WORKERS:-4}"
  --direct-write-workers "${KST_DIRECT_WRITE_WORKERS:-4}"
  --lookup-mode "${KST_LOOKUP_MODE:-interrupt}"
  --commit-mode "${KST_COMMIT_MODE:-interrupt}"
  --shards "${KST_SHARD_COUNT:-4}"
