...
keinctl cluster format --nodes chunky-mcchunkface.example,parity-parrot.example,bytey-mcbyteface.example --confirm
...
```json
{
    "node_id": "answering-machine.example",
...
kix format /dev/nvme0n1 \
    ...
    --node-id answering-machine.example \
    --cluster-id cluster-mcclusterface
...
kix format /dev/nvme2n1 --node-id answering-machine.example --cluster-id cluster-mcclusterface --register
...
Host: quartermaster-quinn.example:8443
    ...
    "targets": [
        {
            ...
            "node": "https://chunky-mcchunkface.example:9443",
            "capability": "eyJ..."
        },
        {
            "chunk_positions": [1, 16, 31, ...],
            "node": "https://parity-parrot.example:9443",
            "capability": "eyJ..."
        }
    ]
}
...
```json
{
    ...
    "version_id": "01HABCD...",
    "chunks": [
        {"index": 0, "node": "https://chunky-mcchunkface.example:9443", "capability": "eyJ..."},
        ...
    ]
}
```
...
The client is in the middle of writing chunks to 15 storage nodes (12+3 profile). Storage node disky-stardust.example crashes after receiving 40 of its 70 assigned chunks.

What happens: the client's connection to disky-stardust.example drops. `libkeinfs` detects the failure (TCP RST or timeout). The client has not yet called commit (Phase 3), so no metadata has been written to the metadata service. The upload is simply abandoned. All chunks written to the 14 surviving nodes, and the 40 chunks written to disky-stardust.example before it died, are orphans. The garbage collector will clean them up on its next sweep (it identifies chunks on storage nodes that have no corresponding object metadata in the metadata service).
...
Optimization: `libkeinfs` can be smarter. Instead of abandoning the entire write, it can re-initiate with a coordinator to get a replacement node for disky-stardust.example, re-send only the chunks that were destined for disky-stardust.example to the new target, and proceed with the commit. This requires the coordinator to issue new capabilities for the replacement node and the client to track which chunks were successfully acknowledged. This is an optimization, not a correctness requirement — the simple "abandon and retry" path is always safe.

**Scenario 2: Storage node dies during a write (Phase 3 — commit)**

All chunks have been written and acknowledged. The client sends the commit request to the coordinator. The coordinator begins the metadata transaction to write the object metadata. At this moment, disky-stardust.example crashes.

What happens: the commit succeeds in the metadata service. The object metadata and chunk map are atomically written. The chunk map correctly records that chunks 2, 17, 32, 47, 62 are on disky-stardust.example. The metadata service does not know or care that disky-stardust.example is dead — it is recording facts about where chunks were written, and those chunks were indeed written and acknowledged before disky-stardust.example died.

Data consistency: **preserved**. The object is committed. The chunks on disky-stardust.example are on disk (they were acknowledged before the crash). When disky-stardust.example restarts, the chunks are still there. If disky-stardust.example never restarts (permanent failure), the rebuilder detects the stale heartbeat, reads the chunk map from the metadata service, reconstructs the 5 missing chunks from the 10 surviving data and parity chunks, writes them to a healthy node, and updates the chunk map in the metadata service. The object is fully available throughout this process (degraded reads work because m=3 and only 1 node is down).
...
Disky-stardust.example crashed, was declared dead, its chunks were rebuilt to other nodes, and now disky-stardust.example comes back online with its old chunks still on disk.

What happens: when disky-stardust.example starts heartbeating again, the rebuilder has already updated the chunk maps in the metadata service to point to the new locations. The chunks on disky-stardust.example are now orphans — they are not referenced by any current chunk map. Disky-stardust.example's local KIX directory may still have entries for these chunks, but the garbage collector's next sweep will identify them as unreferenced (cross-referencing metadata chunk maps with the node's reported chunk inventory) and delete them, freeing the space.
...
{
    "upload_id": "01HXYZ...",
    "node": "chunky-mcchunkface.example",
    "op": "PUT",
    "chunk_positions": [0, 15, 30],
    "expires": 1708300800,
    "sig": "..."
...
```
keinctl node drain answering-machine.example
```
...
**Figure 6.** Example distributed trace for a GET operation, showing span hierarchy from coordinator resolution through parallel storage node reads. The trace immediately identifies disky-stardust.example as the latency outlier at 45 ms.
...
gantt
    ...
    bytey-mcbyteface.example.get_chunk (11 ms)  :s3, 1600, 12600
    chunky-mcchunkface.example.get_chunk (12 ms)  :s1, 1600, 13600
    parity-parrot.example.get_chunk (14 ms)  :s2, 1600, 15600
    disky-stardust.example.get_chunk (45 ms) ⚠️ SLOW :crit, s9, 1600, 46600

    section Client
    ec_decode (ISA-L)        :d1, 46600, 49600
```

This trace tells the operator instantly: the read took 49 ms because storage node disky-stardust.example was slow at 45 ms, while everything else completed in under 15 ms. Cross-referencing disky-stardust.example's drive metrics reveals it was saturated from a concurrent rebuild operation.
...
Diagnosis for train-run-42:
    ⚠ Rank 142 p99 read latency = 850 ms (cluster avg: 45 ms)
        → Rank 142 is reading from storage node disky-stardust.example
        → disky-stardust.example drive nvme2 queue depth = 128 (saturated)
        → disky-stardust.example nvme2 is rebuilding 12,000 chunks (unlucky-seagull.example failure)

    Recommendation:
        Rebuild is impacting disky-stardust.example. Current rebuild throttle: 500 MB/s.
...
```json
{
    ...
    "ec_profile": "12+3",
    "coordinator": "bosun-bubbles.example",
...
// Connect to cluster
keinfs_handle_t *h = keinfs_connect("quartermaster-quinn.example:8443,navigator-nell.example:8443", &opts);
...
let client = Client::connect("quartermaster-quinn.example:8443,navigator-nell.example:8443").await?;
...
client = keinfs.connect("quartermaster-quinn.example:8443,navigator-nell.example:8443")
...
# Mount a bucket
keinfs-fuse mount \
    --coordinators quartermaster-quinn.example:8443,navigator-nell.example:8443 \
    --bucket training-data \
...
export AWS_ENDPOINT_URL=https://bucket-buccaneer.example:8443
