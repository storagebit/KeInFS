...
```
Native control plane:   https://quartermaster-quinn.example:8443/v1/{bucket}/{key}
S3 proxy ingress:       https://bucket-buccaneer.example:8443/{bucket}/{key}
Management API:         https://quartermaster-quinn.example:8443/mgmt/v1/...
```
...
```json
{
    "bindings": [
        {
            "identity": "user:captain@pirate-guild.example",
            "role": "tenant-admin",
            "scope": { "tenant": "acme" }
        },
        {
            "identity": "group:tensor-tamers@pirate-guild.example",
            "role": "team-admin",
        ...
        {
            "identity": "user:deckhand@pirate-guild.example",
            "role": "viewer",
...
```json
{
    ...
    "status": "running",
    "type": "node.drain",
    "target": "answering-machine.example",
    "progress": {
        "chunks_migrated": 0,
        "chunks_total": 145832,
        "percent": 0.0
...
coordinators = [
    "https://quartermaster-quinn.example:8443",
    "https://navigator-nell.example:8443",
    "https://bosun-bubbles.example:8443",
]

# Authentication — choose one method
[auth]
method = "hmac"                                # "hmac", "bearer", or "mtls"
access_key = "<set-via-KEINFS_ACCESS_KEY_ID>"  # For HMAC method
secret_key = "<set-via-KEINFS_SECRET_KEY>"     # For HMAC method
...
The `KEINFS_COORDINATORS` environment variable accepts a comma-separated list of coordinator URLs (e.g., `https://quartermaster-quinn.example:8443,https://navigator-nell.example:8443`). A single URL is valid.
...
```json
{
    "cluster_id": "cluster-mcclusterface",
    "status": "healthy",
    "nodes": {
        "total": 120,
        "healthy": 118,
...
```
Cluster: cluster-mcclusterface                          Status: HEALTHY
    ...
    "failure_domains": [
        {
            ...
            "nodes": [
                {
                    "node_id": "chunky-mcchunkface.example",
                    "status": "healthy",
                    "drives": 8,
...
```json
{
    "nodes": ["chunky-mcchunkface.example", "parity-parrot.example", "bytey-mcbyteface.example"],
...
chunky-mcchunkface.example     healthy      8/8     30.72 TB    18.43 TB    60.0%   5s ago
parity-parrot.example     healthy      8/8     30.72 TB    19.01 TB    61.9%   3s ago
bytey-mcbyteface.example     degraded     7/8     26.88 TB    17.20 TB    64.0%   4s ago
sleepy-spindle.example  dead       0/8       —           —           —     12m ago ⚠
...
```
...
```json
{
    ...
    "hostname": "answering-machine.example",
...
```bash
keinctl node status answering-machine.example
```
...
keinctl node drain answering-machine.example --max-bandwidth 1GB/s --reason "Firmware update"
```
...
```bash
keinctl node rejoin answering-machine.example
```
...
```bash
keinctl drive list
keinctl drive list --node answering-machine.example
...
```bash
keinctl drive status answering-machine.example nvme0
...
keinctl drive format answering-machine.example /dev/nvme0n1
...
# Dry run
keinctl drive format answering-machine.example /dev/nvme0n1 --dry-run

# Force re-format
keinctl drive format answering-machine.example /dev/nvme0n1 --force

# Secure erase + format
keinctl drive format answering-machine.example /dev/nvme0n1 --force --secure-erase
```
...
```bash
keinctl drive inspect answering-machine.example nvme0
```
...
```json
{
    ...
    "audit_reads": false,
    "created_at": "2026-02-19T10:30:05Z",
    "created_by": "user:captain@pirate-guild.example",
    "object_count": 0,
    "size_bytes": 0,
    "tags": {
        "environment": "production",
...
```json
{
    ...
    "admin_contact": "disk-druid@pirate-guild.example",
    "quota": {
...
keinctl tenant create acme \
    ...
    --admin-contact disk-druid@pirate-guild.example \
    --capacity 500TB \
    --bandwidth 200Gbps \
...
```json
{
    ...
    "admin_contact": "model-wranglers@pirate-guild.example",
    "quota": {
        ...
        "findings": [
            {
                ...
                "cluster_average": 45,
                "cause": {
                    "storage_node": "disky-stardust.example",
                    "drive": "nvme2",
                    "issue": "drive_queue_saturated",
                    "detail": "Queue depth 128 (max). Concurrent rebuild of 12,000 chunks from unlucky-seagull.example failure."
                },
                "recommendations": [
                    {
...
{
    "active": true,
    "reason": "Drive failure on disky-stardust.example (nvme2)",
...
{
    "targets": [
        { "component": "coordinator", "instance": "quartermaster-quinn.example", "endpoint": "https://quartermaster-quinn.example:8444/metrics" },
        { "component": "coordinator", "instance": "navigator-nell.example", "endpoint": "https://navigator-nell.example:8444/metrics" },
        { "component": "storage_node", "instance": "chunky-mcchunkface.example", "endpoint": "https://chunky-mcchunkface.example:9444/metrics" },
        { "component": "rebuilder", "instance": "doctor-restripe.example", "endpoint": "https://doctor-restripe.example:9544/metrics" }
    ]
}
```
...
keinctl audit --category management --identity user:captain@pirate-guild.example --period 7d
...
```json
{
    ...
    "sink": {
        "type": "s3",
        "endpoint": "https://archive-ark.example",
...
keinctl audit export --tenant acme --period 2026-01-01,2026-02-01 \
    --sink s3 --sink-endpoint https://archive-ark.example \
...
```json
{
    "access_key_id": "<generated-id-redacted>",
    "secret_key": "<shown-once-redacted>",
    "identity": "serviceaccount:training-pipeline",
    "scope": { "tenant": "acme", "team": "ml-research", "project": "llm-v3" },
    "created_at": "2026-02-19T10:30:05Z",
    "expires_at": "2027-02-19T00:00:00Z"
...
```bash
keinctl auth rbac grant user:deckhand@pirate-guild.example team-admin \
...
```bash
keinctl auth rbac revoke user:deckhand@pirate-guild.example team-admin \
    --tenant acme --team ml-research
```
...
```json
{
    "context_token": "<generated-context-token-redacted>",
