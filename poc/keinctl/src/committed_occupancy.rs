...
mod tests {
    ...
    fn key_roundtrips_target_and_granule() {
        let key = committed_granule_key("chunky-mcchunkface-07", 123_456_789);
        assert_eq!(
            decode_committed_granule_key(&key),
            Some(("chunky-mcchunkface-07".to_string(), 123_456_789))
        );
    }
    ...
    fn target_prefix_groups_only_that_target() {
        let p0 = committed_granule_target_prefix("chunky-mcchunkface-00");
        let p1 = committed_granule_target_prefix("chunky-mcchunkface-01");
        assert!(committed_granule_key("chunky-mcchunkface-00", 5).starts_with(&p0));
        assert!(!committed_granule_key("chunky-mcchunkface-00", 5).starts_with(&p1));
