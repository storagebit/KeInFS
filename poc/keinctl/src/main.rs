...
struct CreateBucketArgs {
    ...
    /// Route objects at or under a size bound to a storage-class profile:
    /// `MAX_BYTES:PROFILE_ID`, repeatable, evaluated ascending by bound. Objects
    /// above every bound use --ec-profile-id.
    #[arg(long = "size-class-route", value_name = "MAX_BYTES:PROFILE_ID")]
    size_class_routes: Vec<String>,
}
            ...
            let reply = rpc_timeout(
                ...
                kms.create_bucket(Request::new(proto::CreateBucketRequest {
                    bucket: Some(proto::BucketRecord {
                        ...
                        size_class_routes: parse_size_class_routes(&args.size_class_routes)?,
                    }),
                })),
            )
            .await?
...
async fn run_target_command(
    ...
    match command {
        TargetCommand::Register(args) => {
            require_confirm(cli)?;
            // KMS owns the target inventory in the decentralized model (KAS removed).
            let mut kms = connect_kms(context, timeout).await?;
            let free_granules = args.free_granules.unwrap_or(args.granule_count);
            let reply = rpc_timeout(
                timeout,
                kms.register_target(Request::new(proto::RegisterTargetRequest {
            ...
            let target = reply
                .target
                .ok_or_else(|| boxed_error("KMS returned no target"))?;
...
async fn collect_target_reports(
    ...
    let targets = rpc_timeout(
        timeout,
        kms.list_targets(Request::new(proto::ListTargetsRequest {})),
    )
    .await?
    .into_inner()
    .targets;
...
mod tests {
    ...
    fn parses_target_register_command() {
        let cli = Cli::try_parse_from([
            ...
            "register",
            "--target-id",
            "chunky-mcchunkface-00",
            "--endpoint",
            "http://203.0.113.1:18080",
            "--server-id",
            "hosty-mchostface.example",
            "--rack-id",
            "rackety-mcrackface",
            "--allocation-shard-id",
            "alloc-shard-of-unusual-size",
            "--granule-count",
            "4096",
        ])
        .expect("CLI should parse");
        ...
        match cli.command {
            TopCommand::Target {
                ...
                assert_eq!(args.target_id, "chunky-mcchunkface-00");
                assert_eq!(args.endpoint, "http://203.0.113.1:18080");
                assert_eq!(args.server_id, "hosty-mchostface.example");
                assert_eq!(args.rack_id, "rackety-mcrackface");
                assert_eq!(args.allocation_shard_id, "alloc-shard-of-unusual-size");
    ...
    fn service_with_live_endpoint_and_no_registry_is_degraded() {
        let report = build_service_status(
            "kms",
            Some("http://198.51.100.11:50060".to_string()),
            None,
            true,
            BTreeMap::new(),
            Vec::new(),
...
/// Parses repeatable `MAX_BYTES:PROFILE_ID` route arguments into proto routes.
fn parse_size_class_routes(raw: &[String]) -> Result<Vec<proto::SizeClassRoute>, DynError> {
    raw.iter()
        .map(|value| {
            let (bytes, profile_id) = value.split_once(':').ok_or_else(|| {
                boxed_error(format!(
                    "size-class route `{value}` is not in MAX_BYTES:PROFILE_ID form"
                ))
            })?;
            let max_object_bytes = bytes.parse::<u64>().map_err(|err| {
                boxed_error(format!("size-class route `{value}` bound is not bytes: {err}"))
            })?;
            if profile_id.is_empty() {
                return Err(boxed_error(format!(
                    "size-class route `{value}` names an empty profile id"
                )));
            }
            Ok(proto::SizeClassRoute {
                max_object_bytes,
                ec_profile_id: profile_id.to_string(),
            })
        })
        .collect()
}
