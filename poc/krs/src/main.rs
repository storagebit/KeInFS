...
async fn execute_placement_task(
    ...
    let write_result = async {
        ...
        let chunk_id = chunk_id_from_proto(&replacement_fragment.chunk_id)
            .map_err(|err| PlacementExecutionError::permanent(err.to_string()))?;
        // Stamp the rebuilt fragment with the original object identity so its on-media
        // self-describing header matches the computed chunk id (and the lease-fenced GC
        // can identify it). object_version_ref is "object_id:version".
        let (object_id, object_version) = parse_object_version_ref(&task.object_version_ref)
            .map_err(PlacementExecutionError::permanent)?;
        let phase_started = Instant::now();
        session
            .write_chunk(
                chunk_id,
                ...
                WriteIdentity {
                    object_id,
                    object_version,
                    stripe: task.stripe_index as u16,
                    frag: task.fragment_index as u16,
                },
...
/// Parses a `version_id` of the form `object_id:version` into its numeric parts.
fn parse_object_version_ref(version_ref: &str) -> Result<(u32, u16), String> {
    let (object_id, version) = version_ref.split_once(':').ok_or_else(|| {
        format!("object_version_ref `{version_ref}` is not in object_id:version form")
    })?;
    let object_id = object_id
        .parse::<u32>()
        .map_err(|err| format!("object_version_ref `{version_ref}` has a bad object_id: {err}"))?;
    let version = version
        .parse::<u16>()
        .map_err(|err| format!("object_version_ref `{version_ref}` has a bad version: {err}"))?;
