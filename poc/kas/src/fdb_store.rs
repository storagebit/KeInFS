...
mod imp {
    ...
    pub(crate) struct FdbKasStore {
        ...
        // Leader-resident-lease mode (see write-scaling design). When set,
    ...
    struct CoordinationLeaseRecord {
        ...
        /// write-scaling design). Bumped ONLY on a *new* grant (an
    ...
    /// mutation epoch (see write-scaling design).
    ///
    ...
    impl FdbKasStore {
        // Lease TTL = 30s, renew at TTL/2 = 15s (write-scaling design §4
        ...
        /// Start the leader-resident lease renewer (write-scaling design §3
        ...
        /// `rewrite_spans` (write-scaling design §3 #2/#3 FIX B). When `true`
        ...
        async fn persist_state(
            ...
            // atomicity is not required in POC state (architecture contract #17): a
        ...
        /// the epoch fence (write-scaling design §3/§4). Reading the lease
        ...
        // ---- Epoch fence + leader-resident lease (write-scaling design §3/§4) ----
        ...
        /// Leader-resident path (flag on, write-scaling design §3 #2/#4):
    ...
    impl AllocatorStore for FdbKasStore {
        ...
        async fn reclaim_target_granules(
            ...
            // FIX A (write-scaling design §3 #2). reclaim genuinely mutates a
    ...
    fn release_record_placements(
        ...
        for placement in released {
            ...
            // re-added). See committed-occupancy design.
    ...
    /// Pure epoch-fence grant decision (see write-scaling design),
    ...
    mod span_neutral_write_tests {
        ...
        fn record_only_target_write() -> TargetWrite {
            TargetWrite {
                record_key: target_key("chunky-mcchunkface-07"),
        ...
        fn span_neutral_write_emits_only_the_record_set() {
            ...
            match &ops[0] {
                WriteOp::Set(key, _) => {
                    assert_eq!(key, &target_key("chunky-mcchunkface-07"));
            ...
            let span_prefix = target_span_prefix("chunky-mcchunkface-07");
            for op in &ops {
                match op {
