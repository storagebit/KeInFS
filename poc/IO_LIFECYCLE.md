...
The diagram and step list below describe the reservation-era window flow,
which remains available as the legacy path. The decentralized path replaces
it: `KSC` calls `KMS BeginObject` (merged begin, which mints the version and
resolves the write context in one RTT), resolves the target roster with
`GetClusterConfig` (cached, epoch-floored), computes placement locally (HRW
over the roster), writes fragments directly, and seals with `CommitObject` —
or `CommitObjectSegmented` (streamed manifest segments for large objects) or
`MultiCommitObject` (several small objects in one RPC). Long-running writes
keep the intent alive with `RenewWriteLease`; a client abandoning a
decentralized write calls `ForfeitObjectWrite` instead of `AbortObjectWrite`.
No `KAS` reservation exists on that path.
...
1. `KSC` calls `KMS ResolveObjectRead`. On the decentralized path the resolve
   RPC is `ResolveObjectHead` instead: it returns the committed head identity
   and profile, and `KSC` derives fragment locations from computed placement
   rather than a stored fragment plan.
...
Above the per-request phases, the object path emits a per-object phase ledger
(the `ksc_object_put_phases_us` / `ksc_object_get_phases_us` print lines and
the benchmark averages). The KMS/EC keys:

- `kms_initiate`
  write path: overall pre-data control-setup wall (contains the two phases
  below plus intent/window handling)
- `kms_begin`
  write path only: the `BeginObject` RPC's own RTT
- `kms_cluster_config`
  write path only: the `GetClusterConfig`/roster resolve (cached hit vs the
  O(targets) KMS roster scan on TTL/epoch miss)
- `kms_commit`
  write path: the commit RPC (`CommitObject` / `CommitObjectSegmented` /
  `MultiCommitObject`, or the window commits on the legacy path)
- `kms_resolve`
  read path only: the object-head/manifest resolve
- `ec_encode`
  write path: Reed–Solomon encode
- `ec_reconstruct`
  read path only: Reed–Solomon reconstruction on degraded reads

The `target_*` keys mirror the per-request phases above, summed across the
fragment fan-out.
...
- `/run/keinfs/kms/kms-<shard-id>-<pid>/summary`
- the per-RPC counters/latencies, keyed by RPC kind — alongside the classic
  kinds (`initiate_object_write`, `commit_object`, `resolve_object_read`,
  ...) the decentralized path adds `resolve_object_head`,
  `commit_object_segmented`, `multi_commit_object`, `renew_write_lease`, and
  `forfeit_object_write`
...
Current validated single-target baseline on `192.0.2.20` for direct single-chunk
`1 MiB` traffic:
