...
use crate::hot_store::{HotMetadataStore, OrphanReclaimDecision, OrphanVersionClaim};
use crate::read_cache::ResolveObjectReadCache;
use crate::stats::{KmsStats, OrphanVersionReapOutcome, RpcKind};
use crate::store::{
...
use keinctl::proto::{
    ...
    CommitObjectReply, CommitObjectRequest, CommitObjectSegment, CommitObjectWriteReply,
    ForfeitObjectWriteReply, ForfeitObjectWriteRequest, ObjectVersionManifest, SizeClassRoute,
    RenewWriteLeaseReply, RenewWriteLeaseRequest,
    MultiCommitObjectReply, MultiCommitObjectRequest, MultiCommitObjectResult,
    ...
    FragmentPlan, GetPlacementTaskReply, GetPlacementTaskRequest, GetTargetPlacementStatusReply,
    GetTargetPlacementStatusRequest, GetWriteIntentReply, GetWriteIntentRequest,
    HeartbeatTargetReply, HeartbeatTargetRequest,
    ...
    ListServiceInstancesRequest, ListTargetsReply, ListTargetsRequest, ListWriteIntentsReply,
    ...
    RegisterTargetReply, RegisterTargetRequest, ReservationMutation, ReserveComputedPlacementReply,
    ReserveComputedPlacementRequest,
    ...
    ResolveShardRequest, RetireTargetReply, RetireTargetRequest, ServiceKind, SetTargetStateReply,
    StripeManifest,
    SetTargetStateRequest, TargetGranule, TargetLifecycleState, TargetRecord, WatchEntryReply,
    WatchEntryRequest, WatchPrefixReply, WatchPrefixRequest, WriteIntent, WriteIntentState,
};
...
/// One occupied granule reported by a target's `/v1/granules/occupied` endpoint. Mirrors
/// the target-side `OccupiedGranuleDocument`; the GC maps the on-media identity to the
/// reverse-log row + write lease and frees the granule (by chunk id + generation) under
/// the lease fence. The wire also carries the granule's `slot`, which the GC does not
/// need (it frees content-addressed), so serde simply ignores it.
#[derive(Debug, Deserialize)]
struct OccupiedGranuleDocument {
    chunk_id: String,
    generation: u32,
    object_id: u32,
    object_version: u32,
...
#[derive(Debug, Deserialize)]
struct OccupiedGranulesPage {
    entries: Vec<OccupiedGranuleDocument>,
...
pub(crate) struct KmsService {
    ...
    /// EC-profile catalog cache with a TTL: profiles created after a fill (e.g. a
    /// new storage class) become routable without a KMS restart.
    pub(crate) ec_profile_catalog: Arc<Mutex<Option<(Instant, Vec<EcProfile>)>>>,
    pub(crate) object_parent_contexts:
        Arc<Mutex<HashMap<ObjectParentCacheKey, ObjectParentContext>>>,
    /// Short-TTL cache of the roster's topology epoch, so the merged BeginObject can
    /// return a REAL epoch (placement-refresh floor for the client) without an
    /// O(targets) inventory scan on every put. The roster changes rarely; a briefly
    /// stale epoch only delays a client's placement refresh by the TTL.
    pub(crate) topology_epoch_cache: Arc<Mutex<Option<(Instant, u64)>>>,
    /// Marker-fenced commits: replace the O(fragments) per-reverse-log-row tombstone
    /// reads in commits and segment appends with one per-version reclaim-fence read.
    /// Only sound once EVERY KMS instance stamps the fence in its granule-GC
    /// authorize path (this build always does), so the flag defaults off and is
    /// flipped after the whole fleet runs fence-writing binaries.
    pub(crate) commit_marker_fence_enabled: bool,
    /// Set true once this instance's startup fence backfill completed: tombstones
    /// stamped by pre-fence binaries have no fence row, so the fenced fast path is
    /// withheld — commits fall back to the tolerant per-row reads, which see those
    /// tombstones directly — until every existing tombstone's version is fenced.
    pub(crate) commit_marker_fence_ready: Arc<std::sync::atomic::AtomicBool>,
}
...
impl Kms for KmsService {
    ...
    async fn delete_namespace_entry(
        ...
        let kind = RpcKind::DeleteNamespaceEntry;
        let started = Instant::now();
        self.stats.record_request(kind);
    ...
    async fn initiate_object_write(
        ...
        let ec_profile = match self
            .select_write_ec_profile(
                kind,
                &default_ec_profile,
                &bucket.size_class_routes,
    ...
    async fn begin_object(
        ...
        let kind = RpcKind::BeginObject;
        let started = Instant::now();
        self.stats.record_request(kind);
        let request = request.into_inner();
        if request.resolve_write_context {
            // Merged begin: ONE RPC + ONE FDB transaction replaces the
            // InitiateObjectWrite/BeginObject pair on the decentralized path. The
            // reply carries the effective (size-routed) profile, stripe count, and
            // parent context; the marker + lease mint and the parent resolution
            // commit atomically; no write-intent row is ever created — the
            // pending-version marker is the write's sole recovery record.
            let phase_started = Instant::now();
            let bucket_context = match self
                .load_bucket_write_context(kind, &request.bucket_id)
                .await
            ...
            let stripe_logical_bytes =
                u64::from(ec_profile.data_fragments) * u64::from(ec_profile.fragment_bytes);
            let stripe_count = request
                .logical_length_bytes
                .div_ceil(stripe_logical_bytes);
            let Ok(stripe_count) = u32::try_from(stripe_count) else {
                return kms_err(
                    &self.stats,
                    kind,
                    &started,
            ...
            let parent_hint = if !normalized_key.contains('/') {
                Some(ObjectParentContext {
                    parent_entry_id: bucket.bucket_entry_id.clone(),
                    parent_path: bucket_entry.path.clone(),
                })
            } else {
                self.lookup_object_parent(&bucket.bucket_id, &normalized_key)
            };
            let lease_expiry =
                now_unix_ms().saturating_add(self.write_intent_ttl.as_millis() as u64);
            let phase_started = Instant::now();
            let (object_id, version, parent_entry_id, parent_path) = match self
                .hot_store
                .begin_object_write_resolved(
                    &bucket.bucket_id,
                    &request.key,
                    &bucket.namespace_id,
            ...
            self.stats
                .record_phase(kind, "store_begin_resolved", phase_started.elapsed());
            self.remember_object_parent(
                &bucket.bucket_id,
                &normalized_key,
                &parent_entry_id,
            ...
            let topology_epoch = self.cached_topology_epoch().await.unwrap_or(0);
            self.stats.record_success(kind, started.elapsed());
            return Ok(Response::new(BeginObjectReply {
                object_id,
                version,
                ec_profile: Some(ec_profile),
                topology_epoch,
        ...
        // Mint-only path (clients that predate the merged begin): unchanged behavior.
        let (object_id, version) = match self
            .hot_store
        ...
        // Durably begin the write attempt: the per-object lease (writer liveness) and
        // the pending-version marker (version liveness + orphan-reaper discovery,
        // carrying the head coordinates for the reaper's liveness cross-check) are
        // written in one transaction. The commit clears both; the reaper claims the
        // marker once the write forfeits.
        let lease_expiry =
            now_unix_ms().saturating_add(self.write_intent_ttl.as_millis() as u64);
        if let Err(err) = self
            .hot_store
    ...
    async fn forfeit_object_write(
        &self,
        request: Request<ForfeitObjectWriteRequest>,
    ) -> Result<Response<ForfeitObjectWriteReply>, Status> {
        // Abandon an in-flight decentralized write: marker -> reclaiming + lease clear
        // in one transaction, so the orphan-version reaper retires the write's rows
        // and granules without waiting out the lease TTL. Idempotent by design.
        let kind = RpcKind::ForfeitObjectWrite;
        let started = Instant::now();
        self.stats.record_request(kind);
        let request = request.into_inner();
        let phase_started = Instant::now();
        match self
            .hot_store
            .forfeit_object_write(request.object_id, request.version)
            .await
        {
            Ok(()) => {
                self.stats
                    .record_phase(kind, "store_forfeit_object_write", phase_started.elapsed());
                self.stats.record_success(kind, started.elapsed());
                Ok(Response::new(ForfeitObjectWriteReply {}))
            }
            Err(err) => {
                self.stats
                    .record_phase(kind, "store_forfeit_object_write", phase_started.elapsed());
    ...
    async fn get_cluster_config(
        ...
        // topology snapshot + epoch, both read from KMS's own target inventory (KAS removed).
        let kind = RpcKind::GetClusterConfig;
        let started = Instant::now();
        self.stats.record_request(kind);
        let phase_started = Instant::now();
        let cluster_salt = match self.hot_store.get_or_init_cluster_salt().await {
            Ok(salt) => salt,
            Err(err) => return kms_err(&self.stats, kind, &started, err),
        };
        self.stats
            .record_phase(kind, "store_get_cluster_salt", phase_started.elapsed());
        let phase_started = Instant::now();
        match self.hot_store.list_targets().await {
            Ok(targets) => {
                self.stats
                    .record_phase(kind, "store_list_targets", phase_started.elapsed());
                let topology_epoch = roster_topology_epoch(&targets);
                self.stats.record_success(kind, started.elapsed());
        ...
        let target = match request.into_inner().target {
            ...
            None => {
                return kms_err(
                    ...
                    kind,
                    &started,
                    Status::invalid_argument("RegisterTarget requires a target"),
                )
            }
        };
        let phase_started = Instant::now();
        match self.hot_store.register_target(target).await {
            Ok(target) => {
                self.stats
    ...
    async fn heartbeat_target(
        ...
        self.stats.record_request(kind);
        let request = request.into_inner();
        let observed = if request.observed_unix_ms == 0 {
            now_unix_ms()
        } else {
        ...
        match self
            .hot_store
            .heartbeat_target(request.target_id, request.healthy, observed)
            .await
        {
            Ok(target) => {
                self.stats
    ...
    async fn set_target_state(
        ...
        match self
            .hot_store
            .set_target_lifecycle(request.target_id, request.lifecycle_state, now_unix_ms())
            .await
        {
            Ok(target) => {
                self.stats
                    .record_phase(kind, "store_set_target_lifecycle", phase_started.elapsed());
                self.stats.record_success(kind, started.elapsed());
    ...
    async fn reserve_computed_placement(
        ...
        let kind = RpcKind::ReserveComputedPlacement;
        let started = Instant::now();
        self.stats.record_request(kind);
        let request = request.into_inner();
        let result: Result<ReserveComputedPlacementReply, Status> = async {
            ...
            Ok(ReserveComputedPlacementReply {
    ...
    async fn commit_object(
        ...
        self.stats.record_request(kind);
        let request = request.into_inner();
        let commit = match self.single_shot_commit_from_request(request) {
            Ok(commit) => commit,
            Err(err) => return kms_err(&self.stats, kind, &started, err),
        };
        let manifest = commit.manifest.clone();
        let phase_started = Instant::now();
        match self.hot_store.commit_object_single_shot(commit).await {
            Ok(head) => {
                // The write lease was cleared inside the commit transaction (atomically
                // with the head flip + reverse-log writes), so no separate clear is needed.
    ...
    async fn commit_object_segmented(
        &self,
        request: Request<Streaming<CommitObjectSegment>>,
    ) -> Result<Response<CommitObjectReply>, Status> {
        // Append-then-seal commit. Each ManifestSegment durably records one batch of stripes'
        // reverse-log + occupancy in its own bounded transaction (the object stays invisible +
        // lease-protected); the terminating Seal is a single-shot commit with EMPTY stripes,
        // whose fence/reverse-log/occupancy loops are no-ops, so it just writes the namespace
        // entry + flips the head + clears the lease — the atomic visibility transition.
        let kind = RpcKind::CommitObjectSegmented;
        let started = Instant::now();
        self.stats.record_request(kind);
        let mut stream = request.into_inner();
        // Split the streamed commit's two halves so neither is dark: the summed per-segment
        // append time (the O(fragments) bulk) and the O(1) seal. Without this the whole stream
        // (measured up to ~210s) was attributed to the RPC total with phases={}.
        let mut append_total = Duration::ZERO;
        // One stream commits exactly one version: the seal must name the version whose
        // segments were appended, or a confused client could flip a fragment-less head.
        let mut appended_version_id: Option<String> = None;
        while let Some(segment) = stream.message().await? {
            match segment.content {
                Some(keinctl::proto::commit_object_segment::Content::ManifestSegment(seg)) => {
                    match &appended_version_id {
                        None => appended_version_id = Some(seg.version_id.clone()),
                        Some(first) if *first != seg.version_id => {
                            return kms_err(
                                &self.stats,
                                kind,
                                &started,
                                Status::invalid_argument(format!(
                                    "segmented commit mixed versions in one stream: segment for {} after segments for {first}",
                                    seg.version_id
                                )),
                            );
                        }
                    ...
                    // Each landed segment pushes the pending-version marker's expiry
                    // forward by a fresh TTL, so a healthy stream never becomes
                    // claim-eligible mid-commit. Computed outside the store's retry
                    // closure (retry-safe clock).
                    let marker_expires_at = now_unix_ms()
                        .saturating_add(self.write_intent_ttl.as_millis() as u64);
                    if let Err(err) = self
                        .hot_store
                        .append_manifest_segment(
                            seg.version_id,
                            seg.stripes,
                            marker_expires_at,
                            self.marker_fence_active(),
                        )
                        .await
                    {
                        return kms_err(&self.stats, kind, &started, err);
                    }
                    append_total = append_total.saturating_add(seg_started.elapsed());
                }
                Some(keinctl::proto::commit_object_segment::Content::Seal(seal)) => {
                    if appended_version_id.as_deref() != Some(seal.version_id.as_str()) {
                        // A seal without prior segments FOR ITS OWN VERSION would flip
                        // a head over zero fragments. No legitimate caller does this
                        // (clients stream segments first); a retried stream re-sends
                        // everything under a fresh write.
                        return kms_err(
                            &self.stats,
                            kind,
                            &started,
                            Status::invalid_argument(format!(
                                "segmented commit sealed version {} but appended segments for {}; the sealed object would have no fragments",
                                seal.version_id,
                                appended_version_id.as_deref().unwrap_or("no version"),
                            )),
                        );
                    }
                    self.stats
                        .record_phase(kind, "append_manifest_segments", append_total);
                    let seal_started = Instant::now();
                    let manifest = ObjectVersionManifest {
                        version_id: seal.version_id,
                        bucket_id: seal.bucket_id.clone(),
                        key: seal.key.clone(),
                        logical_length_bytes: seal.logical_length_bytes,
                        ec_profile_id: seal.ec_profile_id,
                        stripes: Vec::new(),
                        namespace_id: seal.namespace_id.clone(),
                        object_entry_id: seal.object_entry_id,
                        bucket_entry_id: String::new(),
                    };
                    return match self
                        .hot_store
                        .commit_object_single_shot(crate::hot_store::SingleShotCommit {
                            expected_prior_version: seal.expected_prior_version,
                            manifest: manifest.clone(),
                            parent_entry_id: seal.parent_entry_id,
                            parent_path: seal.parent_path,
                            topology_epoch: seal.topology_epoch,
                            // Decentralized path: the reverse-log the segments wrote IS the
                            // manifest, so no manifest blob is persisted.
                            omit_manifest: true,
                            // Sealing streamed segments: the pending-version marker must
                            // still be live, or the orphan reaper won this version.
                            seals_segments: true,
                            require_pending_version_marker: true,
                            marker_fenced: self.marker_fence_active(),
                        })
                        .await
                    {
                        Ok(head) => {
                            self.stats
                                .record_phase(kind, "seal_object", seal_started.elapsed());
                            let key = normalize_object_key(&manifest.key)
                                .unwrap_or_else(|_| manifest.key.clone());
                            invalidate_local_object_cache(&self.read_cache, &manifest.bucket_id, &key);
                            self.notifications.notify(object_invalidation_event(
                                &manifest.namespace_id,
                                &manifest.bucket_id,
                ...
                None => {
                    return kms_err(
                        ...
                        Status::invalid_argument(
                            "KMS CommitObjectSegmented received a segment with no content",
                        ),
                    );
                }
            }
        }
        // The stream ended without a Seal: no head was flipped, so the object is not visible.
        // The appended reverse-log/occupancy rows stay lease-protected until the write retries
        // (a fresh object_id) or the lease lapses and the orphan reaper reclaims them.
        kms_err(
            &self.stats,
            kind,
            &started,
            Status::invalid_argument("KMS CommitObjectSegmented stream ended without a seal"),
        )
    }

    async fn renew_write_lease(
        &self,
        request: Request<RenewWriteLeaseRequest>,
    ) -> Result<Response<RenewWriteLeaseReply>, Status> {
        // Refresh the per-object lease AND the pending-version marker so a long
        // multi-TiB write outlives the base TTL. Refused with FAILED_PRECONDITION once
        // the write forfeits (its marker reaped or being reclaimed): a zombie writer's
        // heartbeat must not resurrect the lease and stall reclamation.
        let kind = RpcKind::RenewWriteLease;
        let started = Instant::now();
        self.stats.record_request(kind);
        let request = request.into_inner();
        let new_expires_at_unix_ms = now_unix_ms().saturating_add(request.lease_ttl_ms);
        let phase_started = Instant::now();
        match self
            .hot_store
            .renew_object_write(request.object_id, new_expires_at_unix_ms)
            .await
        {
            Ok(()) => {
                self.stats
                    .record_phase(kind, "store_renew_object_write", phase_started.elapsed());
                self.stats.record_success(kind, started.elapsed());
                Ok(Response::new(RenewWriteLeaseReply {
    ...
    async fn multi_commit_object(
        &self,
        request: Request<MultiCommitObjectRequest>,
    ) -> Result<Response<MultiCommitObjectReply>, Status> {
        // Batched single-shot commit: the store folds every object's head CAS, reverse log,
        // and namespace writes into ONE FoundationDB transaction (one read-version, one
        // resolver pass, one fsync, one RTT) — amortizing the per-object commit cost that
        // limits the small/medium-object commit rate.
        let kind = RpcKind::MultiCommitObject;
        let started = Instant::now();
        self.stats.record_request(kind);
        let request = request.into_inner();
        if request.commits.is_empty() {
            self.stats.record_success(kind, started.elapsed());
            return Ok(Response::new(MultiCommitObjectReply {
                results: Vec::new(),
            }));
        }
        // Build the batch, keeping each object's (namespace, bucket, normalized key,
        // version) for the post-commit cache-invalidate + watcher notify on success.
        let mut commits = Vec::with_capacity(request.commits.len());
        let mut object_meta = Vec::with_capacity(request.commits.len());
        for commit in request.commits {
            let commit = match self.single_shot_commit_from_request(commit) {
                Ok(commit) => commit,
                Err(err) => return kms_err(&self.stats, kind, &started, err),
            };
            let manifest = &commit.manifest;
            let key =
                normalize_object_key(&manifest.key).unwrap_or_else(|_| manifest.key.clone());
            object_meta.push((
        ...
        let phase_started = Instant::now();
        let outcomes = match self.hot_store.commit_objects_single_shot_batch(commits).await {
            Ok(outcomes) => outcomes,
            Err(err) => {
                self.stats
                    .record_phase(kind, "store_multi_commit_object", phase_started.elapsed());
                return kms_err(&self.stats, kind, &started, err);
            }
        };
        self.stats
            .record_phase(kind, "store_multi_commit_object", phase_started.elapsed());
        // Per-object: on success invalidate the read cache + notify watchers under the
        // normalized key (mirroring commit_object); a per-object Err is carried in its slot.
        let mut results = Vec::with_capacity(outcomes.len());
        for (index, (outcome, (namespace_id, bucket_id, key, version_id, profile_id, logical))) in
            outcomes.into_iter().zip(object_meta.into_iter()).enumerate()
        {
            match outcome {
                Ok(head) => {
                    invalidate_local_object_cache(&self.read_cache, &bucket_id, &key);
                    self.notifications.notify(object_invalidation_event(
                        &namespace_id,
                        &bucket_id,
                        &key,
                    ...
                    self.record_class_ledger(kind, &profile_id, logical, false).await;
                    results.push(MultiCommitObjectResult {
                        index: index as u32,
                        head: Some(head),
                        error: String::new(),
                        grpc_status_code: 0,
                ...
                Err(status) => {
                    // Per-object rejections ride result slots, not kms_err, so the
                    // reclaimed-version counter contract (same phrase match as
                    // kms_err) is applied here per slot — a reaper-claimed or
                    // GC-fenced version must count regardless of the commit route.
                    if status.code() == tonic::Code::FailedPrecondition
                        && status.message().contains("rewritten under a fresh write")
                    {
                        self.stats.record_commit_rejected_version_reclaimed();
                    }
                    results.push(MultiCommitObjectResult {
                        index: index as u32,
                        head: None,
                        error: status.message().to_string(),
                        grpc_status_code: status.code() as i32 as u32,
                    });
                }
            }
        }
        self.stats.record_success(kind, started.elapsed());
        Ok(Response::new(MultiCommitObjectReply { results }))
    }
    ...
    async fn resolve_object_head(
        ...
        let kind = RpcKind::ResolveObjectHead;
        let started = Instant::now();
        self.stats.record_request(kind);
        let request = request.into_inner();
        let phase_started = Instant::now();
            ...
            .await
        {
            Ok(Some(head)) => {
                self.stats
                    .record_phase(kind, "store_get_object_head", phase_started.elapsed());
                head
            }
            Ok(None) => {
                self.stats
            ...
            .await
        {
            Ok(context) => {
                self.stats.record_phase(
                    kind,
                    "store_get_bucket_write_context",
                    phase_started.elapsed(),
                );
                self.stats.record_success(kind, started.elapsed());
                Ok(Response::new(ResolveObjectHeadReply {
                    head: Some(head),
                    ec_profile: Some(context.ec_profile),
                }))
            }
            Err(err) => {
                self.stats.record_phase(
    ...
    async fn delete_object(
        ...
        for version in &deleted.deleted_versions {
            self.record_class_ledger(
                kind,
                &version.manifest.ec_profile_id,
                version.manifest.logical_length_bytes,
                true,
            )
            .await;
        }
...
impl KmsService {
    /// Records one committed/deleted object against its storage class in the
    /// capacity ledger, computing the granule cost from the profile geometry.
    /// A profile missing from the catalog (deleted class, stale cache) skips the
    /// record rather than guessing — the ledger prefers gaps to wrong geometry.
    async fn record_class_ledger(
        &self,
        kind: RpcKind,
        profile_id: &str,
        ...
        let Ok(catalog) = self.list_ec_profiles_cached(kind).await else {
            return;
        };
        let Some(profile) = catalog.into_iter().find(|p| p.id == profile_id) else {
            return;
        };
        let granules = class_granules_for_profile(logical_length_bytes, &profile);
        if deleted {
            self.stats
                .record_class_delete(profile_id, logical_length_bytes, granules);
        } else {
            self.stats
                .record_class_commit(profile_id, logical_length_bytes, granules);
        }
    }

    /// The marker-fenced fast path is active only when the operator enabled it AND
    /// this instance's startup fence backfill completed (pre-fence tombstones all
    /// have fence rows). Until both hold, marker-requiring commits run the tolerant
    /// per-row reads — same rejections, more point reads.
    fn marker_fence_active(&self) -> bool {
        self.commit_marker_fence_enabled
            && self
                .commit_marker_fence_ready
                .load(std::sync::atomic::Ordering::Relaxed)
    }

    /// Converts one wire `CommitObjectRequest` into the store's `SingleShotCommit`.
    /// Two request shapes: the manifest-carrying shape (legacy/tools), and the
    /// landed-rows shape where the server derives the reverse-log/occupancy rows —
    /// the manifest is synthesized here (chunk ids and endpoints are recomputable
    /// pure functions and are never stored, so those fields stay empty). Landed
    /// implies omit_manifest: there is no blob to persist. The marker-fenced fast
    /// path engages only when the client requires the marker AND this KMS has
    /// fencing enabled (`--commit-marker-fence-enabled`).
    fn single_shot_commit_from_request(
        &self,
        request: CommitObjectRequest,
    ) -> Result<crate::hot_store::SingleShotCommit, Status> {
        let (manifest, omit_manifest) = match (request.manifest, request.landed.is_empty()) {
            (Some(_), false) => {
                return Err(Status::invalid_argument(
                    "KMS CommitObject takes either a manifest or landed fragment rows, not both",
                ));
            }
            (Some(manifest), true) => (manifest, request.omit_manifest),
            (None, false) => {
                // A well-formed client writes each (stripe, fragment) once; a
                // duplicate row would silently last-write-win in the transaction's
                // blind sets and could strand a committed-occupancy marker for the
                // losing granule. Reject instead of guessing.
                let mut seen = HashSet::with_capacity(request.landed.len());
                for row in &request.landed {
                    if !seen.insert((row.stripe_index, row.fragment_index)) {
                        return Err(Status::invalid_argument(format!(
                            "KMS CommitObject landed rows repeat stripe {} fragment {}",
                            row.stripe_index, row.fragment_index
                        )));
                    }
                }
                (
                    manifest_from_landed_rows(
                        request.object_id,
                        request.version,
                        request.bucket_id,
                        request.key,
                        request.namespace_id,
                        request.object_logical_length_bytes,
                        request.ec_profile_id,
                        request.object_entry_id,
                        request.landed,
                    ),
                    true,
                )
            ...
            (None, true) => {
                return Err(Status::invalid_argument(
                    "KMS CommitObject requires a manifest or landed fragment rows",
                ));
            }
        };
        Ok(crate::hot_store::SingleShotCommit {
            ...
            omit_manifest,
            seals_segments: false,
            require_pending_version_marker: request.require_pending_version_marker,
            marker_fenced: request.require_pending_version_marker && self.marker_fence_active(),
        })
    }
    ...
    async fn list_ec_profiles_cached(&self, kind: RpcKind) -> Result<Vec<EcProfile>, Status> {
        const EC_PROFILE_CATALOG_TTL: Duration = Duration::from_secs(15 * 60);
        if let Some((cached_at, cached)) = self
            ...
            .clone()
        {
            if cached_at.elapsed() < EC_PROFILE_CATALOG_TTL {
                self.stats
        ...
        *self
            .ec_profile_catalog
            .lock()
            .expect("ec_profile_catalog mutex poisoned") = Some((Instant::now(), profiles.clone()));
        Ok(profiles)
    }
    ...
    async fn select_write_ec_profile(
        ...
        size_class_routes: &[SizeClassRoute],
        logical_length_bytes: u64,
    ) -> Result<EcProfile, Status> {
        // Explicit per-bucket size-class routes take priority over the family-shrink
        // heuristic below, and a routed profile is used verbatim — a replicated
        // small-object class may sit below the family minimum-fragment floor and in a
        // different codec family entirely.
        if !size_class_routes.is_empty() {
            let mut routes = size_class_routes.to_vec();
            routes.sort_by_key(|route| route.max_object_bytes);
            for route in &routes {
                if logical_length_bytes <= route.max_object_bytes {
                    let profile = self
                        .list_ec_profiles_cached(kind)
                        .await?
                        .into_iter()
                        .find(|candidate| candidate.id == route.ec_profile_id)
                        .ok_or_else(|| {
                            Status::failed_precondition(format!(
                                "bucket size-class route ({} bytes) names unknown ec profile {}",
                                route.max_object_bytes, route.ec_profile_id
                            ))
                        })?;
                    self.stats
                        .record_phase(kind, "write_profile_class_route", Duration::ZERO);
                    return Ok(profile);
                }
            }
        }
    ...
    /// The roster topology epoch, cached briefly (see `topology_epoch_cache`).
    async fn cached_topology_epoch(&self) -> Result<u64, Status> {
        const TOPOLOGY_EPOCH_CACHE_TTL: Duration = Duration::from_secs(5);
        if let Some((cached_at, epoch)) = *self
            .topology_epoch_cache
            .lock()
            .expect("topology_epoch_cache mutex poisoned")
        {
            if cached_at.elapsed() < TOPOLOGY_EPOCH_CACHE_TTL {
                return Ok(epoch);
            }
        }
        let targets = self.hot_store.list_targets().await?;
        let epoch = roster_topology_epoch(&targets);
        *self
            .topology_epoch_cache
            .lock()
            .expect("topology_epoch_cache mutex poisoned") = Some((Instant::now(), epoch));
        Ok(epoch)
    }
...
/// FIX C (write-scaling design §4/§7): whether a per-shard reserve error
...
async fn reclaim_target_granules(
    ...
    // FIX A (write-scaling design §3 #2) — KMS-side routing, PARTIAL.
    //
...
/// Active targets paired with their KP2/HTTP-2 endpoints, for the orphan GC to reach each
/// target's occupied-granule scan + generation-fenced free. A target without an endpoint is
/// skipped (it cannot be paged).
/// Content-derived topology epoch of a target roster (identical to what KAS computed),
/// from KMS's own inventory now that KMS owns the roster.
fn roster_topology_epoch(targets: &[TargetRecord]) -> u64 {
    let placement_targets = targets
        .iter()
        .map(keinctl::placement::PlacementTarget::from_record)
        .collect::<Vec<_>>();
    keinctl::placement::topology_epoch(&placement_targets)
}

/// Active targets (healthy + Active + with an endpoint) paired with their KP2/HTTP-2
/// endpoints, read from KMS's own inventory, for the orphan GC to reach each target.
async fn list_active_targets_with_endpoints(
    ...
    Ok(hot_store
        ...
        .map(|target| (target.target_id, target.endpoint))
        .collect())
}

/// Tallies of one orphan-GC sweep across all active targets.
#[derive(Default)]
pub(crate) struct OrphanGcSweepSummary {
    pub(crate) freed: u64,
    pub(crate) skipped_committed: u64,
    pub(crate) lease_active: u64,
    pub(crate) reused: u64,
}

/// Runs one lease-fenced orphan-GC sweep: for each active target, page its occupied
/// granules and, per granule, authorize a reclaim in FoundationDB (the reverse-log
/// rendezvous + lease check) and — when authorized — free it on the target under the
/// generation fence. Bounded by `max_granules_per_target` so a sweep is O(roster) work.
/// Per-target errors are logged and skipped so one unreachable target does not stall the
/// rest; the lease-fenced commit path keeps committed data safe regardless.
///
/// `resume_cursors` carries the per-target paging cursor ACROSS sweeps so a full drive is
/// walked over successive sweeps rather than re-examining only its lowest-slot granules
/// every time. A target that is fully walked in one sweep is dropped from the map so the
/// next sweep restarts it from the beginning (catching newly-orphaned granules).
pub(crate) async fn run_orphan_gc_sweep(
    hot_store: std::sync::Arc<dyn HotMetadataStore>,
    lease_grace_ms: u64,
    max_granules_per_target: usize,
    resume_cursors: &mut HashMap<String, u64>,
    stats: std::sync::Arc<KmsStats>,
) -> Result<OrphanGcSweepSummary, Status> {
    const OCCUPIED_PAGE_LIMIT: usize = 256;
    // Snapshot the clock ONCE for the whole sweep so every reclaim decision is judged
    // against the same instant (and the FDB closure, which may re-run on retry, never
    // re-reads it).
    let now_unix_ms = now_unix_ms();
    let targets = list_active_targets_with_endpoints(&hot_store).await?;
    let mut summary = OrphanGcSweepSummary::default();
    for (target_id, endpoint) in targets {
        let session = match TargetDeleteSession::connect(&endpoint).await {
            Ok(session) => session,
            Err(err) => {
                stats.set_last_error(format!(
                    "orphan GC could not reach target {target_id} at {endpoint}: {err}"
                ));
                continue;
            }
        };
        // Resume where the previous sweep left this target (None = from the start).
        let mut cursor: Option<u64> = resume_cursors.get(&target_id).copied();
        let mut examined = 0usize;
        let mut fully_walked = false;
        'target: loop {
            if examined >= max_granules_per_target {
                break;
            }
            let page = match session.list_occupied_granules(cursor, OCCUPIED_PAGE_LIMIT).await {
                Ok(page) => page,
                Err(err) => {
                    stats.set_last_error(format!(
                        "orphan GC occupied-granule scan of target {target_id} failed: {err}"
                    ));
                    break;
                }
            };
            for entry in &page.entries {
                examined += 1;
                let decision = hot_store
                    .authorize_orphan_reclaim(
                        target_id.clone(),
                        entry.object_id,
                    ...
                    .await?;
                match decision {
                    OrphanReclaimDecision::Committed => summary.skipped_committed += 1,
                    OrphanReclaimDecision::LeaseActive => summary.lease_active += 1,
                    OrphanReclaimDecision::Authorized
                    | OrphanReclaimDecision::AlreadyReclaimed => {
                        let chunk_id = match hex::decode(&entry.chunk_id) {
                            Ok(bytes) => bytes,
                            Err(err) => {
                                stats.set_last_error(format!(
                                    "orphan GC got a malformed chunk id from target {target_id}: {err}"
                                ));
                                continue;
                            }
                        };
                        match session.free_orphan_granule(&chunk_id, entry.generation).await {
                            Ok(true) => summary.freed += 1,
                            Ok(false) => summary.reused += 1,
                            Err(err) => {
                                stats.set_last_error(format!(
                                    "orphan GC could not free granule on target {target_id}: {err}"
                                ));
                                // A free error means the target is unhealthy; abandon it for
                                // this sweep (the reverse-log tombstone persists, so a later
                                // sweep retries the free) and move on to the next target.
                                break 'target;
                            }
                        }
                    }
        ...
        // Persist the resume point: drop it when the target was fully walked (restart from
        // the beginning next sweep) or never advanced; otherwise continue from `cursor`.
        match cursor {
            Some(next) if !fully_walked => {
                resume_cursors.insert(target_id, next);
            }
            _ => {
                resume_cursors.remove(&target_id);
            }
        }
    }
    stats.record_orphan_gc_run(
        summary.freed,
        summary.skipped_committed,
...
/// Raw pending-version rows (markers + presence rows) read per discovery scan.
const ORPHAN_VERSION_SCAN_ROWS_PER_TXN: usize = 4_096;
/// Versions claimed or resumed per sweep.
const ORPHAN_VERSION_CLAIMS_PER_TICK: usize = 32;
/// Reverse-log rows examined per cleaning transaction (~3 clears per row, far under
/// the FDB per-transaction byte/time budgets).
const ORPHAN_VERSION_ROWS_PER_CLEAN_TXN: usize = 2_048;
/// Cleaning transactions per sweep, bounding a reap storm's write pressure.
const ORPHAN_VERSION_CLEAN_TXNS_PER_TICK: usize = 64;

/// One orphan-version reaper sweep: discover claim-eligible pending versions, claim
/// each (`writing -> reclaiming`), clean their reverse-log/occupancy/committed-granule
/// rows per presence target in bounded transactions, and delete the finished markers.
/// The physical granules are then freed by the ordinary granule GC, whose decision
/// table reaches `Authorized` once the rows are gone. Work left over when a budget
/// runs out resumes next sweep via the durable `reclaiming` markers + presence rows;
/// a single version's failure never aborts the sweep.
pub(crate) async fn run_orphan_version_reap(
    hot_store: std::sync::Arc<dyn HotMetadataStore>,
    claim_grace_ms: u64,
    lease_grace_ms: u64,
    scan_cursor: &mut Option<Vec<u8>>,
    stats: std::sync::Arc<KmsStats>,
) -> Result<(), Status> {
    // One clock snapshot for the whole sweep (FDB closures may re-run on retry and
    // must never re-read the clock).
    let now_unix_ms = now_unix_ms();
    let (mut candidates, next_cursor, undecodable_rows) = hot_store
        .scan_pending_versions(
            scan_cursor.take(),
            ORPHAN_VERSION_SCAN_ROWS_PER_TXN,
            now_unix_ms,
            claim_grace_ms,
        )
        .await?;
    let mut outcome = OrphanVersionReapOutcome {
        backlog: candidates.len() as u64,
        oldest_age_ms: candidates
            .iter()
            .map(|candidate| now_unix_ms.saturating_sub(candidate.expires_at_unix_ms))
            .max()
            .unwrap_or(0),
        // Undecodable marker rows can never be reaped; surface them as errors
        // rather than silently skipping a corrupt keyspace forever.
        errors: undecodable_rows,
        ..Default::default()
    };
    // Crash-resume work first, so an interrupted clean can never be starved by a
    // steady stream of freshly-expired versions.
    candidates.sort_by_key(|candidate| {
        candidate.state != crate::store::PENDING_VERSION_STATE_RECLAIMING
    });
    let mut clean_budget = ORPHAN_VERSION_CLEAN_TXNS_PER_TICK;
    for candidate in candidates.into_iter().take(ORPHAN_VERSION_CLAIMS_PER_TICK) {
        if clean_budget == 0 {
            break;
        }
        let targets = match hot_store
            .claim_orphan_version(
                candidate.object_id,
            ...
            .await
        {
            ...
            Ok(OrphanVersionClaim::Resumed { targets }) => {
                outcome.resumed += 1;
                targets
            }
            Ok(OrphanVersionClaim::SkippedWriterAlive) => {
                outcome.skipped_writer_alive += 1;
                continue;
            }
            Ok(OrphanVersionClaim::SkippedHeadLive) => {
                outcome.skipped_head_live += 1;
                continue;
            }
            Ok(OrphanVersionClaim::Gone) => continue,
            Err(err) => {
                outcome.errors += 1;
                stats.set_last_error(format!(
                    "orphan-version claim of {}:{} failed: {err}",
                    candidate.object_id, candidate.object_version
                ));
                continue;
            }
        };
        let mut all_targets_done = true;
        'targets: for resume in targets {
            // Continue from the durable cursor the target's last partial clean
            // recorded in its presence row, not from the range start — the granule
            // GC re-stamps tombstones into cleared rows behind the cleaner, so a
            // restart-from-start could spend every sweep's budget re-paging the
            // tombstone prefix without ever reaching the uncleared tail.
            let mut page_cursor: Option<Vec<u8>> = resume.cursor;
            loop {
                if clean_budget == 0 {
                    // Budget exhausted: the marker stays reclaiming, presence rows
                    // record the remaining targets + cursors, and the next sweep
                    // resumes.
                ...
                clean_budget -= 1;
                match hot_store
                    .clean_orphan_version_target_page(
                        candidate.object_id,
                        candidate.object_version,
                        resume.target_id.clone(),
                        page_cursor.take(),
                        ORPHAN_VERSION_ROWS_PER_CLEAN_TXN,
                    )
                    .await
                {
                    Ok(page) => {
                        outcome.rows_cleared += page.rows_cleared;
                        outcome.kco1_cleared += page.kco1_cleared;
                        outcome.tombstones_skipped += page.tombstones_skipped;
                        outcome.errors += page.unrecognized_skipped;
                        if page.target_done {
                            outcome.targets_cleaned += 1;
                            break;
                        }
                    ...
                    Err(err) => {
                        outcome.errors += 1;
                        stats.set_last_error(format!(
                            "orphan-version clean of {}:{} on target {} failed: {err}",
                            candidate.object_id, candidate.object_version, resume.target_id
                        ));
                        all_targets_done = false;
                        break 'targets;
                    }
        ...
        if all_targets_done {
            match hot_store
                .finish_orphan_version(candidate.object_id, candidate.object_version)
                .await
            {
                Ok(true) => outcome.reaped += 1,
                // Presence rows remain (a concurrent instance is mid-clean, or a
                // clean raced a fresh append fence): the next sweep resumes it.
                Ok(false) => {}
                Err(err) => {
                    outcome.errors += 1;
                    stats.set_last_error(format!(
                        "orphan-version finish of {}:{} failed: {err}",
                        candidate.object_id, candidate.object_version
                    ));
                }
            }
    ...
    *scan_cursor = next_cursor;
    stats.record_orphan_version_reap(&outcome);
    Ok(())
}
...
impl TargetDeleteSession {
    ...
    /// Generation-fenced free of an orphan granule: the target deletes the chunk only if
    /// its granule still holds exactly `(chunk_id, generation)`. Returns whether the
    /// target actually freed it (false = the granule was already gone or reused).
    async fn free_orphan_granule(
        &self,
        chunk_id: &[u8],
        generation: u32,
    ) -> Result<bool, Status> {
        ...
        let mut ready = self.client.clone().ready().await.map_err(|err| {
            Status::unavailable(format!(
                "KMS could not ready HTTP/2 stream for orphan free {}: {}",
                uri, err
            ))
        })?;
        let mut request = HttpRequest::builder()
            ...
            .map_err(|err| {
                Status::internal(format!(
                    "KMS could not construct orphan free request {}: {}",
                    uri, err
                ))
            })?;
        request
            ...
            .insert(CONTENT_LENGTH, http::HeaderValue::from_static("0"));
        let (response_future, _) = ready.send_request(request, true).map_err(|err| {
            Status::unavailable(format!("KMS could not send orphan free request {}: {}", uri, err))
        })?;
        let response = response_future
            .await
            .map_err(|err| Status::unavailable(format!("KMS orphan free {} failed: {}", uri, err)))?;
        let status = response.status();
        let body = collect_h2_body(response.into_body()).await.map_err(|err| {
            Status::unavailable(format!("KMS could not read orphan free response {}: {}", uri, err))
        })?;
        if status != StatusCode::OK {
            return Err(Status::internal(format!(
                "KMS orphan free {} returned {} with body {}",
                uri,
                status,
                String::from_utf8_lossy(&body)
            )));
        ...
        let document: DeleteChunkDocument = serde_json::from_slice(&body).map_err(|err| {
            Status::internal(format!(
                "KMS could not decode orphan free response {}: {}",
                uri, err
            ))
        })?;
        Ok(document.deleted)
    ...
    /// Fetches one page of the target's occupied granules (slot + chunk id + generation +
    /// on-media object identity), resuming after `cursor` (exclusive) and examining at most
    /// `limit` granules. The reply's `next_cursor` is the resume point, or None at the end.
    async fn list_occupied_granules(
        &self,
        cursor: Option<u64>,
        limit: usize,
    ) -> Result<OccupiedGranulesPage, Status> {
        let mut uri = format!(
            "{}/v1/granules/occupied?limit={}",
            self.endpoint.trim_end_matches('/'),
            limit
        );
        ...
        let mut ready = self.client.clone().ready().await.map_err(|err| {
            Status::unavailable(format!(
                "KMS could not ready HTTP/2 stream for occupied-granule scan {}: {}",
                uri, err
            ))
        })?;
        let request = HttpRequest::builder()
            ...
            .map_err(|err| {
                Status::internal(format!(
                    "KMS could not construct occupied-granule request {}: {}",
                    uri, err
                ))
            })?;
        let (response_future, _) = ready.send_request(request, true).map_err(|err| {
            Status::unavailable(format!(
                "KMS could not send occupied-granule request {}: {}",
                uri, err
            ))
        })?;
        let response = response_future.await.map_err(|err| {
            Status::unavailable(format!("KMS occupied-granule scan {} failed: {}", uri, err))
        })?;
        let status = response.status();
        let body = collect_h2_body(response.into_body()).await.map_err(|err| {
            Status::unavailable(format!(
                "KMS could not read occupied-granule response {}: {}",
                uri, err
            ))
        })?;
        if status != StatusCode::OK {
            return Err(Status::internal(format!(
                "KMS occupied-granule scan {} returned {} with body {}",
                uri,
                status,
                String::from_utf8_lossy(&body)
            )));
        ...
        serde_json::from_slice(&body).map_err(|err| {
            Status::internal(format!(
                "KMS could not decode occupied-granule response {}: {}",
                uri, err
            ))
        })
    }
...
fn kms_err<T>(
    ...
    // A FAILED_PRECONDITION carrying the store's reclaimed-version rejection text
    // means the orphan-version reaper won this write (commit/append/seal arrived
    // after the version forfeited). Counted distinctly from generic RPC errors: a
    // steady climb means writers are outliving their leases — heartbeats too sparse
    // or the TTL too short — and losing whole writes. Matched on the shared phrase
    // every reclaimed-version rejection message ends with; keep the two in sync.
    if err.code() == tonic::Code::FailedPrecondition
        && err.message().contains("rewritten under a fresh write")
    {
        stats.record_commit_rejected_version_reclaimed();
    }
...
/// Physical slot count an object occupies under `profile`: every stripe consumes
/// (data + parity) whole granules regardless of padding — which is exactly the
/// cost the capacity ledger exists to surface (replicated 3x, stripe-padding
/// floors, sub-extent slot waste).
fn class_granules_for_profile(logical_length_bytes: u64, profile: &EcProfile) -> u64 {
    let fragments =
        u64::from(profile.data_fragments.max(1)) + u64::from(profile.parity_fragments);
    stripe_count_for_profile(logical_length_bytes, profile) as u64 * fragments
}
...
/// Rebuilds an in-memory manifest from a commit's landed fragment rows: rows group
/// by stripe (ascending) and sort by fragment index within each stripe, so the
/// synthesized manifest is deterministic regardless of the wire order the client's
/// fan-out completed in. Chunk ids and endpoints stay empty — they are recomputable
/// pure functions and nothing in the commit transaction reads them.
#[allow(clippy::too_many_arguments)]
fn manifest_from_landed_rows(
    object_id: u32,
    version: u32,
    bucket_id: String,
    key: String,
    ...
    logical_length_bytes: u64,
    ec_profile_id: String,
    object_entry_id: String,
    landed: Vec<keinctl::proto::LandedFragmentRow>,
) -> ObjectVersionManifest {
    let mut stripes: BTreeMap<u32, Vec<FragmentPlan>> = BTreeMap::new();
    for row in landed {
        stripes
            .entry(row.stripe_index)
            .or_default()
            .push(FragmentPlan {
                ...
                endpoint: String::new(),
                granule_index: row.granule_index,
    ...
    let stripes = stripes
        .into_values()
        .map(|mut fragments| {
            fragments.sort_unstable_by_key(|fragment| fragment.fragment_index);
            StripeManifest { fragments }
        })
        .collect();
    ObjectVersionManifest {
        version_id: format!("{object_id}:{version}"),
        bucket_id,
        key,
        logical_length_bytes,
        ec_profile_id,
...
mod tests {
        ...
        merge_discovered_allocation_shard_routes(
            ...
            &[
                keinctl::proto::ServiceInstanceRecord {
                    ...
                    service_kind: ServiceKind::Kas as i32,
                    node_id: "quartermaster-quinn.example".to_string(),
                ...
                keinctl::proto::ServiceInstanceRecord {
                    ...
                    service_kind: ServiceKind::Kas as i32,
                    node_id: "navigator-nell.example".to_string(),
        ...
        merge_discovered_allocation_shard_routes(
            ...
            &[
                ...
                keinctl::proto::ServiceInstanceRecord {
                    ...
                    service_kind: ServiceKind::Kas as i32,
                    node_id: "bosun-bubbles.example".to_string(),
    ...
    fn route_change_invalidation_covers_failover_signals() {
        // FIX C (write-scaling design §4/§7): with the always-on epoch fence,
    ...
    #[test]
    fn landed_rows_synthesize_deterministic_manifest() {
        use keinctl::proto::LandedFragmentRow;
        // Rows arrive in the order the client's fan-out happened to complete:
        // stripes and fragments interleaved. The synthesized manifest must come
        // out grouped by ascending stripe and sorted by fragment within a stripe.
        let landed = vec![
            LandedFragmentRow {
        ...
        let manifest = manifest_from_landed_rows(
            7,
            1,
            "bkt".to_string(),
            "dir/obj.bin".to_string(),
            "ns".to_string(),
            65536,
            "rep-1-2".to_string(),
            String::new(),
        ...
        assert_eq!(manifest.version_id, "7:1");
        assert_eq!(manifest.logical_length_bytes, 65536);
        assert_eq!(manifest.ec_profile_id, "rep-1-2");
        assert_eq!(manifest.stripes.len(), 2);
        let flat: Vec<(u32, u32, &str, u64, u32)> = manifest
            .stripes
            .iter()
            .flat_map(|stripe| {
        ...
        // Chunk ids/endpoints are never stored: recomputable pure functions.
        for stripe in &manifest.stripes {
            for fragment in &stripe.fragments {
                assert!(fragment.chunk_id.is_empty());
                assert!(fragment.endpoint.is_empty());
            }
        }
    }
}
