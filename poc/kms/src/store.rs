        ...
        self.db
            .run(move |trx, _| {
                ...
                async move {
                    ...
                    // above (an object's fragments may appear in both). Reclaim
                    // tombstones are permanent residue of freed granules, not live
                    // fragments — skipping them keeps dead versions from consuming
                    // the max_versions budget and starving rebuild task creation.
                    let mut stream =
...
pub(crate) fn write_target_current_fragment_index(
    ...
    for stripe in &manifest.stripes {
        for fragment in &stripe.fragments {
            ...
            // that already holds committed data. See committed-occupancy design.
            trx.set(
...
/// A reverse-log row holding this one-byte sentinel marks the
/// (target, version_id, stripe, frag) granule as reclaimed by the lease-fenced GC: an
/// uncommitted orphan whose physical granule has been (or is being) freed. It is
/// length-distinguishable from a committed value, which is always 12 bytes. The GC
/// writes it inside the same transaction that reads the row absent + the lease expired,
/// so a concurrent commit (which reads the same row) read-conflicts and exactly one wins.
pub(crate) const REVERSE_LOG_RECLAIM_TOMBSTONE: &[u8] = &[0xFF];

/// True when a reverse-log value is the GC reclaim tombstone (see
/// [`REVERSE_LOG_RECLAIM_TOMBSTONE`]). A commit that finds this on any of its fragments
/// must abort — the granule's bytes are gone.
#[allow(dead_code)]
pub(crate) fn is_reverse_log_reclaim_tombstone(bytes: &[u8]) -> bool {
    bytes == REVERSE_LOG_RECLAIM_TOMBSTONE
}
...
/// One reverse-log value, classified. Shared by the orphan-version cleaner (which must
/// recover the granule_index to pair the KCO1 clear) and every consumer that filters
/// tombstones, so the two can never disagree about what a row means.
#[derive(Debug, PartialEq, Eq)]
pub(crate) enum ReverseLogRowValue {
    Committed { generation: u32, granule_index: u64 },
    ReclaimTombstone,
    Unrecognized,
}

pub(crate) fn classify_reverse_log_value(bytes: &[u8]) -> ReverseLogRowValue {
    if is_reverse_log_reclaim_tombstone(bytes) {
        return ReverseLogRowValue::ReclaimTombstone;
    }
    if bytes.len() != 12 {
        return ReverseLogRowValue::Unrecognized;
    }
    let generation = u32::from_be_bytes(bytes[0..4].try_into().expect("4-byte generation"));
    let granule_index = u64::from_be_bytes(bytes[4..12].try_into().expect("8-byte granule"));
    ReverseLogRowValue::Committed {
        generation,
        granule_index,
    }
}

/// Extracts `(object_id, object_version)` from a `version_id` of the form
/// `object_id:version` (the deterministic id the decentralized commit path mints).
/// CANONICAL ONLY: the raw string is embedded verbatim in reverse-log/occupancy keys
/// while marker keys and the reaper's range reconstruction use the parsed integers, so
/// a non-canonical spelling (`"007:5"`, `"+7:5"` — bare u32 parsing accepts both) would
/// desync the two keyspaces and strand rows no reaper range can ever find.
pub(crate) fn parse_version_id_parts(version_id: &str) -> Result<(u32, u32), Status> {
    version_id
        .split_once(':')
        .and_then(|(object_id, version)| {
            Some((object_id.parse::<u32>().ok()?, version.parse::<u32>().ok()?))
        })
        .filter(|(object_id, version)| {
            format!("{object_id}:{version}") == version_id
        })
        .ok_or_else(|| {
            Status::invalid_argument(format!(
                "manifest version_id `{version_id}` is not in canonical object_id:version form"
            ))
        })
}

pub(crate) const PENDING_VERSION_STATE_WRITING: u8 = 0x01;
pub(crate) const PENDING_VERSION_STATE_RECLAIMING: u8 = 0x02;
const PENDING_VERSION_VALUE_TAG: u8 = 0x01;

/// A pending-version marker's value. `bucket_id`/`key` exist solely so the reaper can
/// cross-check head liveness before claiming (and for log context); both may be empty
/// on markers minted by the backfill. The key stored is the NORMALIZED object key —
/// the same form every commit path passes to `object_head_key` — otherwise the
/// liveness check would silently look up a head that can never exist.
...
pub(crate) fn encode_pending_version_value(pending: &PendingVersion) -> Vec<u8> {
    let mut out = Vec::with_capacity(18 + pending.bucket_id.len() + pending.key.len());
    out.push(PENDING_VERSION_VALUE_TAG);
    out.push(pending.state);
    out.extend_from_slice(&pending.expires_at_unix_ms.to_be_bytes());
    let bucket_len = u32::try_from(pending.bucket_id.len()).expect("bucket_id length fits u32");
    out.extend_from_slice(&bucket_len.to_be_bytes());
    out.extend_from_slice(pending.bucket_id.as_bytes());
    let key_len = u32::try_from(pending.key.len()).expect("key length fits u32");
    out.extend_from_slice(&key_len.to_be_bytes());
    out.extend_from_slice(pending.key.as_bytes());
    out
}

pub(crate) fn decode_pending_version_value(bytes: &[u8]) -> Result<PendingVersion, Status> {
    let fail = |detail: &str| {
        Status::internal(format!("pending-version marker value is invalid: {detail}"))
    };
    if bytes.len() < 18 {
        return Err(fail(&format!("{} bytes, expected at least 18", bytes.len())));
    }
    if bytes[0] != PENDING_VERSION_VALUE_TAG {
        return Err(fail(&format!("unknown layout tag {}", bytes[0])));
    }
    let state = bytes[1];
    if state != PENDING_VERSION_STATE_WRITING && state != PENDING_VERSION_STATE_RECLAIMING {
        return Err(fail(&format!("unknown state {state}")));
    }
    let expires_at_unix_ms =
        u64::from_be_bytes(bytes[2..10].try_into().expect("8-byte expiry"));
    let read_string = |offset: usize| -> Result<(String, usize), Status> {
        if offset + 4 > bytes.len() {
            return Err(fail("truncated string length"));
        }
        let len = u32::from_be_bytes(bytes[offset..offset + 4].try_into().expect("4-byte len"))
            as usize;
        let end = offset + 4 + len;
        if end > bytes.len() {
            return Err(fail("truncated string payload"));
        }
        let value = std::str::from_utf8(&bytes[offset + 4..end])
            .map_err(|err| fail(&format!("non-utf8 string: {err}")))?;
        Ok((value.to_string(), end))
    };
    let (bucket_id, offset) = read_string(10)?;
    let (key, offset) = read_string(offset)?;
    if offset != bytes.len() {
...
/// True once a pending-version marker's protection has lapsed: its expiry plus the
/// claim grace has passed. Extracted pure so the boundary is unit-tested once and
/// shared by discovery and the claim's in-transaction re-check.
pub(crate) fn pending_version_claim_eligible(
...
mod tests {
    use super::{
        apply_fragment_repair, build_finalize_plans_for_reservations, encode_reverse_log_value,
        is_reverse_log_reclaim_tombstone, REVERSE_LOG_RECLAIM_TOMBSTONE,
    };
    use keinctl::proto::{
    ...
    #[test]
    fn reclaim_tombstone_is_distinguishable_from_every_committed_value() {
        // The GC reclaim tombstone must never collide with a committed reverse-log value,
        // or a commit could mistake a live fragment for reclaimed (or vice versa). A
        // committed value is always 12 bytes; the tombstone is one byte.
        assert!(is_reverse_log_reclaim_tombstone(REVERSE_LOG_RECLAIM_TOMBSTONE));
        for &(generation, granule) in &[
            (0u32, 0u64),
            (1, 7),
            (u32::MAX, u64::MAX),
            (0xFF, 0xFF),
            ...
            let committed = encode_reverse_log_value(generation, granule);
            assert_eq!(committed.len(), 12);
            assert!(
                !is_reverse_log_reclaim_tombstone(&committed),
                "committed value for ({generation}, {granule}) must not read as a tombstone"
            );
        }
        // An empty value (absent row materialized as empty) is not a tombstone either.
        assert!(!is_reverse_log_reclaim_tombstone(&[]));
    }

    #[test]
    fn reverse_log_values_classify_as_committed_tombstone_or_unrecognized() {
        use super::{classify_reverse_log_value, ReverseLogRowValue};
        assert_eq!(
            classify_reverse_log_value(&encode_reverse_log_value(3, 42)),
            ReverseLogRowValue::Committed {
                generation: 3,
                granule_index: 42
            }
        ...
        assert_eq!(
            classify_reverse_log_value(REVERSE_LOG_RECLAIM_TOMBSTONE),
            ReverseLogRowValue::ReclaimTombstone
        );
        for garbage in [&[][..], &[0u8; 5][..], &[0u8; 13][..]] {
            assert_eq!(
                classify_reverse_log_value(garbage),
    ...
    #[test]
    fn pending_version_value_round_trips() {
        use super::{
            decode_pending_version_value, encode_pending_version_value, PendingVersion,
            PENDING_VERSION_STATE_RECLAIMING, PENDING_VERSION_STATE_WRITING,
        };
        for pending in [
            PendingVersion {
                expires_at_unix_ms: 1_750_000_000_000,
                state: PENDING_VERSION_STATE_WRITING,
                bucket_id: "vault16".to_string(),
                key: "checkpoint/epoch-7".to_string(),
            },
            // Backfilled markers carry no head context.
            PendingVersion {
            ...
            let decoded =
                decode_pending_version_value(&encode_pending_version_value(&pending)).unwrap();
            assert_eq!(decoded, pending);
        }
    }

    #[test]
    fn pending_version_value_rejects_malformed_bytes() {
        use super::{
            decode_pending_version_value, encode_pending_version_value, PendingVersion,
            PENDING_VERSION_STATE_WRITING,
        };
        let valid = encode_pending_version_value(&PendingVersion {
            expires_at_unix_ms: 5,
            state: PENDING_VERSION_STATE_WRITING,
            bucket_id: "b".to_string(),
            key: "k".to_string(),
        });
        // Truncations at every boundary fail rather than misdecode.
        for cut in [0, 1, 2, 9, 10, 13, valid.len() - 1] {
            assert!(decode_pending_version_value(&valid[..cut]).is_err());
        }
        // Unknown layout tag.
        let mut tagged = valid.clone();
        tagged[0] = 0x7f;
        assert!(decode_pending_version_value(&tagged).is_err());
        // Unknown state byte.
        let mut misstated = valid.clone();
        misstated[1] = 0x7f;
        assert!(decode_pending_version_value(&misstated).is_err());
        // Trailing bytes are rejected (a marker is never a prefix of another value).
        let mut extended = valid.clone();
        extended.push(0);
        assert!(decode_pending_version_value(&extended).is_err());
    }

    #[test]
    fn pending_version_claim_eligibility_boundaries() {
        use super::{
            pending_version_claim_eligible, PendingVersion, PENDING_VERSION_STATE_WRITING,
        };
        let pending = |expires_at_unix_ms| PendingVersion {
            expires_at_unix_ms,
            state: PENDING_VERSION_STATE_WRITING,
            bucket_id: String::new(),
            key: String::new(),
        };
        // Eligible exactly when expiry + grace <= now.
        assert!(pending_version_claim_eligible(&pending(100), 400, 300));
        assert!(!pending_version_claim_eligible(&pending(101), 400, 300));
        assert!(pending_version_claim_eligible(&pending(0), 0, 0));
        // A huge expiry saturates instead of wrapping into premature eligibility.
        assert!(!pending_version_claim_eligible(
            &pending(u64::MAX - 1),
            u64::MAX - 1,
            u64::MAX
        ));
    ...
    #[test]
    fn version_id_parts_round_trip_with_marker_keys() {
        use super::parse_version_id_parts;
        // The marker key stores hex segments while version_id strings are decimal;
        // this pins the conversion round-trip both ways.
        let (object_id, object_version) = parse_version_id_parts("4097:12").unwrap();
        assert_eq!((object_id, object_version), (4097, 12));
        let version_id = format!("{object_id}:{object_version}");
        assert_eq!(version_id, "4097:12");
        let key = crate::fdb_schema::pending_version_key(object_id, object_version);
        let (decoded_id, decoded_version, target) =
            crate::fdb_schema::decode_pending_version_key_parts(&key).unwrap();
        assert_eq!((decoded_id, decoded_version, target), (4097, 12, None));
        for malformed in [
            "4097", "a:1", "1:a", ":", "4294967296:1",
            // Non-canonical spellings parse as u32 but would desync the raw-string
            // row keys from the integer-derived marker keys — rejected.
            "007:5", "+7:5", "7:+5", "7:05",
        ] {
            assert!(parse_version_id_parts(malformed).is_err(), "{malformed}");
        }
    }
        ...
        WriteIntent {
            ...
            fragment_plans: vec![
                ...
                FragmentPlan {
                    ...
                    endpoint: "http://tiny-target-two.example".to_string(),
                    granule_index: 12,
                    generation: 1,
                    stripe_index: 0,
                },
        ...
        let replacement = PlacementReservationRecord {
            ...
            placements: vec![
                PlacementReservation {
                    ...
                    endpoint: "http://tiny-target-nine.example".to_string(),
                    granule_index: 99,
                    fragment_index: 0,
                    reservation_id: "res-new".to_string(),
                    reservation_placement_index: 0,
                ...
                PlacementReservation {
                    ...
                    endpoint: "http://tiny-target-eight.example".to_string(),
                    granule_index: 98,
                    fragment_index: 1,
                    reservation_id: "res-new".to_string(),
                    reservation_placement_index: 1,
