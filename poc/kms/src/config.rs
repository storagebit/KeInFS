...
pub(crate) struct Config {
    ...
    /// FoundationDB client network threads for this process. 1 keeps the stock
    /// single-threaded client. Above 1, libfdb runs this many client threads (each
    /// loading its own copy of the client library from fdb_external_client_dir) and
    /// the store spreads database handles across them — the single network thread
    /// otherwise caps a busy KMS at a few thousand transactions per second.
    pub(crate) fdb_client_threads: usize,
    /// Directory holding a copy of libfdb_c.so for the multi-threaded client.
    /// Required when fdb_client_threads > 1.
    pub(crate) fdb_external_client_dir: String,
    /// Spawns the orphan-version reaper (reclaims segmented writes that died before
    /// their seal). PRECONDITION: every KMS instance in the fleet must already run a
    /// binary that mints/clears pending-version markers, or the reaper can misread
    /// writes driven through older binaries. Default false; enable fleet-wide only
    /// after the binary rollout completes (fresh labs can enable it from day one).
    pub(crate) orphan_version_reaper_enabled: bool,
    /// Marker-fenced commits: commits/appends that require the pending-version
    /// marker replace their O(fragments) per-reverse-log-row tombstone reads with
    /// one per-version reclaim-fence read. PRECONDITION: every KMS instance in the
    /// fleet must already run a binary whose granule-GC authorize path stamps the
    /// reclaim fence — a tombstone stamped through an older instance's GC while
    /// fenced commits are served is invisible to them. Tombstones that predate the
    /// rollout are handled automatically: each start with the flag on backfills
    /// their fences and withholds the fenced fast path (tolerant per-row reads
    /// keep serving) until the backfill completes. Default false; enable only once
    /// no pre-fence binary still runs a GC loop (fresh labs: from day one).
...
struct FileConfig {
    ...
    orphan_version_reaper_enabled: Option<bool>,
    commit_marker_fence_enabled: Option<bool>,
    fdb_client_threads: Option<usize>,
    fdb_external_client_dir: Option<String>,
}
...
impl Config {
    fn defaults() -> Result<Self, Box<dyn Error>> {
        Ok(Self {
            ...
            orphan_version_reaper_enabled: false,
            commit_marker_fence_enabled: false,
            fdb_client_threads: 1,
            fdb_external_client_dir: String::new(),
        })
    }
    ...
    fn apply_file(&mut self, path: &Path) -> Result<(), Box<dyn Error>> {
        ...
        if let Some(value) = file.orphan_version_reaper_enabled {
            self.orphan_version_reaper_enabled = value;
        }
        if let Some(value) = file.commit_marker_fence_enabled {
            self.commit_marker_fence_enabled = value;
        }
        if let Some(value) = file.fdb_client_threads {
            self.fdb_client_threads = value.max(1);
        }
        if let Some(value) = file.fdb_external_client_dir {
            self.fdb_external_client_dir = value;
        }
    ...
    pub(crate) fn fingerprint_source(&self) -> String {
        format!(
            concat!(
                ...
                "orphan_version_reaper_enabled={}\n",
                "commit_marker_fence_enabled={}\n",
                "fdb_client_threads={}\n",
                "fdb_external_client_dir={}\n"
            ),
            self.listen_addr,
            ...
            self.orphan_version_reaper_enabled,
            self.commit_marker_fence_enabled,
            self.fdb_client_threads,
            self.fdb_external_client_dir,
        )
    }
}
...
pub(crate) fn parse_args(args: Vec<String>) -> Result<Config, Box<dyn Error>> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--orphan-version-reaper-enabled" => {
                i += 1;
                let value = args
                    .get(i)
                    .ok_or_else(|| missing_value("--orphan-version-reaper-enabled"))?;
                config.orphan_version_reaper_enabled =
                    parse_bool_arg(value, "--orphan-version-reaper-enabled")?;
            }
            "--commit-marker-fence-enabled" => {
                i += 1;
                let value = args
                    .get(i)
                    .ok_or_else(|| missing_value("--commit-marker-fence-enabled"))?;
                config.commit_marker_fence_enabled =
                    parse_bool_arg(value, "--commit-marker-fence-enabled")?;
            }
            "--fdb-client-threads" => {
                i += 1;
                config.fdb_client_threads = args
                    .get(i)
                    .ok_or_else(|| missing_value("--fdb-client-threads"))?
                    .parse::<usize>()?
                    .max(1);
            }
            "--fdb-external-client-dir" => {
                i += 1;
                config.fdb_external_client_dir = args
                    .get(i)
                    .ok_or_else(|| missing_value("--fdb-external-client-dir"))?
                    .clone();
            }
            flag => {
...
mod tests {
    ...
    fn parse_foundationdb_args() {
        let parsed = parse_args(vec![
            ...
            "192.0.2.7:50061,192.0.2.8:50061".to_string(),
