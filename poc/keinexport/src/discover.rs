...
pub fn discover(roots: &[PathBuf]) -> Vec<Instance> {
    ...
    for root in roots {
        ...
        for svc_entry in entries.flatten() {
            ...
            // dir contains all 12 targets: chunky-mcchunkface-00, -01, ...). Each
...
fn freshest_instances(svc_path: &Path, service: &str) -> Vec<Instance> {
    ...
    for inst_entry in entries.flatten() {
        ...
        let mut summary = inst_path.join("summary");
        // The client-side benchmark (`ksc object-benchmark --stats-root`, and kfc's
        // equivalent) publishes its live JSON snapshot as `object-benchmark.json`
        // rather than `summary`; accept it so the client tier is scrapeable.
        if !summary.is_file() && matches!(service, "ksc" | "kfc") {
            summary = inst_path.join("object-benchmark.json");
        }
...
/// instance share a prefix. `chunky-mcchunkface-00-2512955` -> `chunky-mcchunkface-00`;
...
mod tests {
    ...
    fn stable_prefix_strips_pid_only_when_an_instance_id_remains() {
        ...
        assert_eq!(stable_prefix("chunky-mcchunkface-00-2512955"), "chunky-mcchunkface-00");
    ...
    fn discovers_all_distinct_instances_and_skips_restart_dupes() {
        ...
        for target in ["chunky-mcchunkface-00", "chunky-mcchunkface-01", "chunky-mcchunkface-02"] {
