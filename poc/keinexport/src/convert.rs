...
pub fn json_snapshot_to_metrics(
    ...
    for (key, val) in obj {
        match key.as_str() {
            ...
            "class_ledger" => {
                if let Some(classes) = val.as_object() {
                    for (class_name, class_val) in classes {
                        emit_class_ledger(service, &base, class_name, class_val, out);
                    }
                }
            }
...
/// Per-storage-class capacity ledger: each profile's committed/deleted objects,
/// logical bytes, and granules become counters labeled `class=<profile-id>`.
fn emit_class_ledger(
    service: &str,
    base: &[(String, String)],
    class_name: &str,
    class_val: &serde_json::Value,
    out: &mut MetricSet,
) {
    let obj = match class_val.as_object() {
        Some(o) => o,
        None => return,
    };
    let mut labels = base.to_vec();
    labels.push(("class".to_string(), class_name.to_string()));
    for field in [
        "committed_objects",
        ...
        if let Some(n) = obj.get(field).and_then(as_number) {
            out.add(
                &format!("keinfs_{service}_class_{field}_total"),
                "counter",
                &format!("Capacity ledger {field} per storage class"),
                labels.clone(),
                n,
            );
        }
...
mod tests {
    ...
    fn kst_kv_summary_and_rpc_phases_become_metrics() {
        ...
        let mut set = MetricSet::new();
        kst_kv_to_metrics(
            "chunky-mcchunkface-00-2512955",
            summary,
            &[("write".to_string(), write_rpc.to_string())],
            &mut set,
        );
        let text = set.render();
        assert!(text.contains("keinfs_kst_write_payload_bytes{service=\"kst\",instance=\"chunky-mcchunkface-00-2512955\",target_id=\"chunky-mcchunkface-00\"} 1862708232192"));
