...
use ksc::object::{
    ...
    DEFAULT_WRITE_INFLIGHT_ENDPOINTS, DEFAULT_WRITE_WINDOW_INFLIGHT_STRIPES,
    DEFAULT_WRITE_WINDOW_MAX_STRIPES,
};
...
pub(crate) struct ObjectPutConfig {
    ...
    pub(crate) write_inflight_endpoints: usize,
    ...
    pub(crate) fault_skip_commit: bool,
}
...
pub(crate) struct ObjectBenchmarkConfig {
    ...
    pub(crate) input_path: PathBuf,
    pub(crate) workers: usize,
    // How many operations each worker keeps in flight at once (1 = the classic
    // sequential worker). At K>1 a worker runs K clients and refills as ops
    // complete, so per-object control latency overlaps across neighboring
    // objects — the measurable knob for how much object rate cross-object
    // pipelining recovers.
    pub(crate) inflight_per_worker: usize,
    // When > 1, each worker drives a library PutPipeline of this depth (write-only
    // benches): the production API for cross-object pipelining, vs the raw
    // client-pool loop --inflight-per-worker exercises.
    ...
    // When set, overrides the client's per-object payload-cache admission cap.
    // 0 keeps read benchmarks honest: every read fetches fragments from the
    // targets instead of replaying the client's whole-object payload cache.
    pub(crate) read_payload_cache_max_object_bytes: Option<usize>,
    pub(crate) write_window_max_stripes: usize,
    pub(crate) write_window_inflight_stripes: usize,
    pub(crate) write_inflight_endpoints: usize,
    pub(crate) kms_grpc_max_message_bytes: usize,
    pub(crate) metadata_notification_nats_url: Option<String>,
    pub(crate) metadata_notification_subject: String,
    // Coalesce up to this many object-head commits into one MultiCommitObject RPC (one FDB
    // txn) across all benchmark workers. 1 (default) keeps the per-object CommitObject path.
    pub(crate) commit_batch_max: usize,
    // Drive the decentralized write/read path (computed placement + target-local
    // allocation, no central reservation) so the benchmark measures the KAS-less
    // metadata path. Default off preserves the legacy reserve-window path.
    pub(crate) decentralized: bool,
...
fn parse_put_object_args(args: &[String]) -> Result<ObjectPutConfig, Box<dyn Error>> {
    ...
    let mut write_inflight_endpoints = DEFAULT_WRITE_INFLIGHT_ENDPOINTS;
    ...
    let mut decentralized = true;
    let mut fault_skip_commit = false;
    let mut i = 0;
    while i < args.len() {
        match args[i].as_str() {
            "--kms-endpoint" => {
            ...
            "--write-inflight-endpoints" => {
                i += 1;
                write_inflight_endpoints = args
                    .get(i)
                    .ok_or_else(|| missing_value("--write-inflight-endpoints"))?
                    .parse()?;
            }
...
fn parse_object_benchmark_args(args: &[String]) -> Result<ObjectBenchmarkConfig, Box<dyn Error>> {
    ...
    let mut read_payload_cache_max_object_bytes = None;
    let mut write_window_max_stripes = DEFAULT_WRITE_WINDOW_MAX_STRIPES;
    let mut write_window_inflight_stripes = DEFAULT_WRITE_WINDOW_INFLIGHT_STRIPES;
    let mut write_inflight_endpoints = DEFAULT_WRITE_INFLIGHT_ENDPOINTS;
    let mut commit_batch_max = 1usize;
    let mut inflight_per_worker = 1usize;
        ...
        match args[i].as_str() {
            ...
            "--inflight-per-worker" => {
                ...
                inflight_per_worker = args
                    .get(i)
                    .ok_or_else(|| missing_value("--inflight-per-worker"))?
                    .parse()?;
                if inflight_per_worker == 0 {
                    return Err(arg_error("--inflight-per-worker must be at least 1"));
                }
            }
            "--pipeline-depth" => {
                i += 1;
                pipeline_depth = args
                    .get(i)
                    .ok_or_else(|| missing_value("--pipeline-depth"))?
                    .parse()?;
                if pipeline_depth == 0 {
                    return Err(arg_error("--pipeline-depth must be at least 1"));
                }
            }
            "--ops-per-worker" => {
                i += 1;
            ...
            "--read-payload-cache-max-object-bytes" => {
                i += 1;
                read_payload_cache_max_object_bytes = Some(
                    args.get(i)
                        .ok_or_else(|| missing_value("--read-payload-cache-max-object-bytes"))?
                        .parse()?,
                );
            }
            "--write-window-max-stripes" => {
            ...
            "--commit-batch-max" => {
                i += 1;
                commit_batch_max = args
                    .get(i)
                    .ok_or_else(|| missing_value("--commit-batch-max"))?
                    .parse()?;
            }
            "--kms-grpc-max-message-bytes" => {
                i += 1;
...
mod tests {
    ...
    #[test]
    fn object_benchmark_accepts_inflight_per_worker() {
        let command = parse_ok(&[
            "object-benchmark",
            "--bucket",
            "lab-8p2",
        ...
        assert_eq!(config.inflight_per_worker, 3);
        // Default is the classic sequential worker; zero is rejected outright.
        let command = parse_ok(&[
            "object-benchmark",
            "--bucket",
            "lab-8p2",
        ...
        assert_eq!(config.inflight_per_worker, 1);
        let argv = [
            "object-benchmark",
            "--bucket",
            "lab-8p2",
        ...
        .collect::<Vec<_>>();
        let err = parse_args(argv).expect_err("zero in-flight must be rejected");
        assert!(err.to_string().contains("--inflight-per-worker"));
    }
    ...
    fn object_benchmark_accepts_multiple_kms_endpoints() {
        let command = parse_ok(&[
            "object-benchmark",
            "--kms-endpoint",
            "http://192.0.2.1:50060,http://192.0.2.2:50060",
            "--kms-endpoint",
            "http://192.0.2.3:50060",
            "--bucket",
            "lab-8p2",
            "--input",
            "/tmp/input.bin",
        ...
        assert_eq!(
            ...
            vec![
                "http://192.0.2.1:50060".to_string(),
                "http://192.0.2.2:50060".to_string(),
                "http://192.0.2.3:50060".to_string()
            ]
        );
    }
...
fn put_object_usage() -> &'static str {
    concat!(
        ...
        "  --write-inflight-endpoints <n> max concurrent fragment writes (adaptive-limiter ceiling), default 64; raise for a single large object\n",
        ...
        "  --fault-skip-commit   DEBUG: write fragments then skip CommitObject (orphan granules for GC validation)\n",
    )
}
...
fn object_benchmark_usage() -> &'static str {
    concat!(
        ...
        "  --read-payload-cache-max-object-bytes <n> per-object payload-cache admission cap; 0 disables client payload caching so reads hit the targets, default 16777216\n",
        "  --write-window-max-stripes <n> max stripes reserved per write window, default 4096\n",
        "  --write-window-inflight-stripes <n> concurrent stripe uploads per window batch, default 16\n",
        "  --kms-grpc-max-message-bytes <n> control-plane gRPC message cap, default 134217728\n",
        "  --metadata-notification-nats-url <uri> subscribe to KMS invalidation events via NATS\n",
