...
pub(crate) async fn run_put_object(
    ...
    // Bracket the WHOLE put so `object_total_us - sum(phase_us)` reveals any client-side dark
    // time (setup, EC spawn-queue, permit-gate, the un-phased gaps). Without this the phase
    // ledger is the only record and dark hops vanish silently rather than showing as a gap.
    ...
    let result = put_object_from_path_with_options(
        ...
        ObjectClientOptions {
            ...
            fault_skip_commit: config.fault_skip_commit,
            ..ObjectClientOptions::default()
        },
    )
    .await?;
    let object_total_us = object_started.elapsed().as_micros();
    println!(
        "ksc_object_put bucket={} key={} logical_bytes={} object_total_us={} version_id={} intent_id={} stripes={} fragments={}",
        config.bucket_id,
        config.key,
