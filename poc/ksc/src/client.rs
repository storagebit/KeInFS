...
// Keepalive: PING each pooled connection on this interval so it stays permanently alive (no
// OS/middlebox idle-reap between bursty checkpoint writes) and so its PING->PONG RTT is a live
// liveness/latency signal — the data-plane analogue of the KMS gRPC keepalive.
const H2_KEEPALIVE_INTERVAL: Duration = Duration::from_secs(15);

/// Live health of a single pooled h2 connection to a target. Lock-free; updated by the
/// connection's keepalive task + driver, snapshotted by the observability emitter.
#[derive(Debug, Default)]
pub struct ConnMetrics {
    /// Most recent PING->PONG round-trip in microseconds (0 until the first pong).
    pub last_ping_rtt_us: AtomicU64,
    /// Successful keepalive pings since connect.
    pub ping_count: AtomicU64,
    /// Pings that errored (connection dead/closing).
    pub ping_failures: AtomicU64,
    /// True while the connection driver future runs; false once the connection is gone.
    pub alive: AtomicBool,
}

/// Process-wide registry of every pooled connection's metrics, so the observability emitter can
/// report connection state + ping latency without threading handles through the write path.
fn conn_metrics_registry() -> &'static Mutex<Vec<(String, Arc<ConnMetrics>)>> {
    static REGISTRY: OnceLock<Mutex<Vec<(String, Arc<ConnMetrics>)>>> = OnceLock::new();
    REGISTRY.get_or_init(|| Mutex::new(Vec::new()))
}

/// Registers a freshly-established connection's metrics so the observability emitter can report
/// its state + ping latency. Called once per connection in `connect_with_options`.
pub fn register_conn_metrics(endpoint: &str, metrics: Arc<ConnMetrics>) {
    if let Ok(mut reg) = conn_metrics_registry().lock() {
        reg.push((endpoint.to_string(), metrics));
    }
}

/// Per-endpoint connection-health snapshot for the observability emitter.
#[derive(Debug)]
pub struct EndpointConnStats {
    pub endpoint: String,
    pub total_conns: usize,
    pub alive_conns: usize,
    pub ping_rtt_us_min: u64,
    pub ping_rtt_us_avg: u64,
    pub ping_rtt_us_max: u64,
    pub ping_failures: u64,
}

/// Aggregates the registry into per-endpoint connection-health snapshots (for live observability).
pub fn snapshot_conn_stats() -> Vec<EndpointConnStats> {
    let Ok(reg) = conn_metrics_registry().lock() else {
        return Vec::new();
    };
    let mut by: std::collections::BTreeMap<String, (usize, usize, u64, u64, u64, u64, u64)> =
        std::collections::BTreeMap::new();
    for (endpoint, m) in reg.iter() {
        let rtt = m.last_ping_rtt_us.load(Ordering::Relaxed);
        let entry = by.entry(endpoint.clone()).or_insert((0, 0, u64::MAX, 0, 0, 0, 0));
        entry.0 += 1; // total
        if m.alive.load(Ordering::Relaxed) {
            entry.1 += 1; // alive
        }
        if rtt > 0 {
            entry.2 = entry.2.min(rtt); // min
            entry.3 = entry.3.max(rtt); // max
            entry.4 += rtt; // sum
            entry.5 += 1; // rtt samples
        }
        entry.6 += m.ping_failures.load(Ordering::Relaxed);
    }
    by.into_iter()
        .map(|(endpoint, (total, alive, min, max, sum, n, fails))| EndpointConnStats {
            endpoint,
            total_conns: total,
            alive_conns: alive,
            ping_rtt_us_min: if n == 0 { 0 } else { min },
            ping_rtt_us_avg: if n == 0 { 0 } else { sum / n },
            ping_rtt_us_max: max,
            ping_failures: fails,
        })
        .collect()
...
/// Outcome of a single direct chunk write. Beyond the phase timings, it reports where
/// the fragment actually landed: with target-local allocation the target chooses the
/// granule, so the writer learns the chosen `granule_index` (and `generation`) from the
/// reply rather than dictating it. The granule equals the one the client requested when
/// the write carried a concrete granule rather than `kp2::GRANULE_ALLOCATE`.
#[derive(Clone, Debug)]
pub struct WriteChunkOutcome {
    pub phases: RequestPhaseTimes,
    // Read by the object write path (lib); the benchmark bin only consumes `phases`.
    #[allow(dead_code)]
    pub granule_index: u64,
    #[allow(dead_code)]
    pub generation: u32,
...
impl TargetSession {
    ...
    pub async fn connect_with_options(
        ...
        let (client, mut connection) = builder.handshake(socket).await.map_err(|err| {
            ClientError::Transport(format!(
        ...
        register_conn_metrics(endpoint, Arc::clone(&metrics));
        // Take the PING/PONG handle BEFORE spawning the driver, so the keepalive task can ping
        // while the driver future drives the connection's frames.
        let ping_pong = connection.ping_pong();
        let driver_metrics = Arc::clone(&metrics);
        tokio::spawn(async move {
            let _ = connection.await;
            // The connection ended (drop/GOAWAY/error): mark it dead for observability and to
            // stop the keepalive task below.
            driver_metrics.alive.store(false, Ordering::Relaxed);
        });
        if let Some(mut ping_pong) = ping_pong {
            let ping_metrics = Arc::clone(&metrics);
            let ping_endpoint = endpoint.to_string();
            tokio::spawn(async move {
                let mut ticker = tokio::time::interval(H2_KEEPALIVE_INTERVAL);
                ticker.tick().await; // first tick is immediate; skip it (just connected)
                loop {
                    ticker.tick().await;
                    if !ping_metrics.alive.load(Ordering::Relaxed) {
                        break;
                    }
                    ...
                    match ping_pong.ping(h2::Ping::opaque()).await {
                        Ok(_pong) => {
                            ...
                            ping_metrics.ping_count.fetch_add(1, Ordering::Relaxed);
                        }
                        Err(_) => {
                            // A failed ping means the connection is gone; record it and stop. The
                            // pool's reconnect path will replace it on the next failed write.
                            let _ = ping_endpoint;
                            ping_metrics.ping_failures.fetch_add(1, Ordering::Relaxed);
                            ping_metrics.alive.store(false, Ordering::Relaxed);
                            break;
                        }
                    }
                }
    ...
    pub async fn write_chunk(
        ...
        payload: Vec<u8>,
    ) -> Result<WriteChunkOutcome, ClientError> {
        let path = format!(
        ...
        // The reply reports the granule the fragment actually landed at (target-chosen
        // under allocate-mode; the requested granule otherwise) plus its generation.
        let landed_granule = required_header_u64(&headers, HEADER_GRANULE_INDEX)
            .or_else(|_| required_header_u64(&headers, HEADER_SLOT_INDEX))
            .map_err(|err| {
                ClientError::Protocol(format!(
                    "KSC write response did not carry a valid granule header: {}",
                    err
                ))
            })?;
        let landed_generation = required_header_u32(&headers, HEADER_GENERATION).map_err(|err| {
            ClientError::Protocol(format!(
                "KSC write response did not carry a valid {} header: {}",
                HEADER_GENERATION, err
            ))
        })?;
        Ok(WriteChunkOutcome {
            phases,
            granule_index: landed_granule,
