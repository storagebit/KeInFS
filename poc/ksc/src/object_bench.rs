...
use ksc::object::{
    new_shared_shard_freelists, ObjectClient, ObjectClientOptions, ObjectPhaseTimes, PutPipeline,
    SharedShardFreelists,
};
...
pub(crate) async fn run_object_benchmark(
    ...
    let client_options = worker_client_options(&config);
    // One recycling free-list registry for EVERY client this run creates
    // (prefill, warm, and all workers x inflight depth), so encode buffers
    // recycle through one bounded pool per profile instead of each client
    // retaining its own working set — the memory discipline the pipelined
    // depths are measured under.
    let shared_freelists = new_shared_shard_freelists();
    ...
    for warm_index in 0..config.kms_endpoints.len().max(1) {
        let mut warm_client = ObjectClient::connect_with_options_sharing_shard_pool(
            &config.kms_endpoints,
            client_options.clone(),
            shared_freelists.clone(),
        )
    ...
    for worker_index in 0..config.workers {
        ...
        let worker_freelists = shared_freelists.clone();
    ...
    println!(
        "ksc_object_benchmark mode={} run_mode={} workers={} ops_per_worker={} warmup_ms={} total_ops={} logical_bytes={} throughput_mib_s={:.2} duration_ms={} errors={} read_key_source={} prefill_keys={} prefill_ms={} write_key_count={} key_prefix={} key_shape={} read_completion_mode={} write_completion_mode={} read_resolve_cache_ttl_ms={} read_payload_cache_max_object_bytes={}",
        ...
        worker_client_options(&config).read_payload_cache_max_object_bytes,
    );
    let (rss_mib, rss_peak_mib) = process_rss_mib();
    println!(
        "ksc_object_benchmark_client inflight_per_worker={} total_clients={} rss_mib={} rss_peak_mib={}",
        config.inflight_per_worker,
        config.workers * config.inflight_per_worker,
        rss_mib,
        rss_peak_mib,
    );
...
/// Client-process VmRSS/VmHWM in MiB from /proc/self/status, so memory growth
/// under pipelined depths is part of every run's record (the write-RAM history
/// makes RSS a merge gate, not an afterthought). (0, 0) where procfs is absent.
fn process_rss_mib() -> (u64, u64) {
    let Ok(status) = std::fs::read_to_string("/proc/self/status") else {
        return (0, 0);
    };
    let mut rss_kib = 0_u64;
    let mut hwm_kib = 0_u64;
    for line in status.lines() {
        let slot = if line.starts_with("VmRSS:") {
            &mut rss_kib
    ...
    (rss_kib / 1024, hwm_kib / 1024)
}

/// The shared client options every benchmark client (prefill, warm, worker,
/// pipelined-depth) is constructed with.
fn worker_client_options(config: &ObjectBenchmarkConfig) -> ObjectClientOptions {
    let defaults = ObjectClientOptions::default();
    ObjectClientOptions {
        ...
        read_payload_cache_max_object_bytes: config
            .read_payload_cache_max_object_bytes
            .unwrap_or(defaults.read_payload_cache_max_object_bytes),
        ...
        write_inflight_endpoints: config.write_inflight_endpoints,
        ...
        commit_batch_max: config.commit_batch_max,
...
async fn run_worker(
    ...
    shared_freelists: SharedShardFreelists,
) -> Result<WorkerTotals, Box<dyn std::error::Error + Send + Sync>> {
    if config.pipeline_depth > 1 {
        if config.write_percent != 100 {
            return Err(std::io::Error::other(
                "--pipeline-depth drives the write pipeline; use --write-percent 100",
            )
            .into());
        }
        return run_worker_putpipeline(
            worker_index,
            config,
            run_prefix,
            payload,
    ...
    if config.inflight_per_worker > 1 {
        return run_worker_pipelined(
            worker_index,
            config,
            run_prefix,
            prefill_keys,
    ...
    let mut totals = WorkerTotals::default();
    let mut client = ObjectClient::connect_with_options_sharing_shard_pool(
        &config.kms_endpoints,
        worker_client_options(&config),
        shared_freelists,
    )
    .await
...
/// A worker that drives the library [`PutPipeline`] — the production API for
/// cross-object write pipelining (depth-bounded, byte-budgeted, same-key
/// serialized). Write-only: each submission's latency is measured from submit
/// admission to its ticket resolving at commit.
async fn run_worker_putpipeline(
    worker_index: usize,
    config: ObjectBenchmarkConfig,
    run_prefix: String,
    payload: Vec<u8>,
    ...
    let mut pipeline = PutPipeline::connect(
        &config.kms_endpoints,
        worker_client_options(&config),
    ...
    let mut pending: std::collections::VecDeque<(Instant, ksc::object::PutTicket)> =
        std::collections::VecDeque::new();
    let mut op_index = 0_usize;
    // Timed mode only for the pipeline bench (fixed-ops uses the client-pool path);
    // warmup is skipped — the pipeline's first `depth` submissions self-warm.
    let ObjectBenchmarkRunMode::Timed { duration, .. } = config.run_mode else {
        return Err(std::io::Error::other(
            "--pipeline-depth requires --duration-secs (timed mode)",
        )
        .into());
    };
    if let Some(barrier) = bench_barrier {
    ...
    while Instant::now() < deadline {
        ...
        match pipeline.submit(&config.bucket_id, &key, payload.clone()).await {
            Ok(ticket) => pending.push_back((submitted, ticket)),
            Err(err) => {
                totals.writes.errors.push(err.to_string());
                live_stats.record_error();
            }
        }
        // Harvest tickets in submission order as they resolve, keeping backlog flat.
        while pending.len() > config.pipeline_depth {
            let (started, ticket) = pending.pop_front().expect("pending ticket");
            match ticket.wait().await {
                Ok(result) => {
                    let elapsed = started.elapsed();
                    record_success(&mut totals.writes, payload_len, elapsed, result.phases);
                    live_stats.record_write(payload_len, elapsed);
                }
                Err(err) => {
                    totals.writes.errors.push(err.to_string());
                    live_stats.record_error();
...
/// A worker that keeps `inflight_per_worker` operations in flight at once: it
/// owns that many clients, runs each op as a spawned task, and refills a slot
/// the moment an op completes. This overlaps neighboring objects' control RTTs
/// and data phases from ONE logical submitter — the measurable preview of what
/// library-level put pipelining recovers of the per-object dark time.
#[allow(clippy::too_many_arguments)]
async fn run_worker_pipelined(
    ...
    let depth = config.inflight_per_worker;
    let options = worker_client_options(&config);
    let mut idle: Vec<ObjectClient> = Vec::with_capacity(depth);
    for _ in 0..depth {
        idle.push(
            ObjectClient::connect_with_options_sharing_shard_pool(
                &config.kms_endpoints,
    ...
    let run_prefix = Arc::new(run_prefix);
    let prefill_keys = Arc::new(prefill_keys);
    let payload = Arc::new(payload);
    let mut totals = WorkerTotals::default();
    let mut joins: JoinSet<(ObjectClient, WorkerTotals)> = JoinSet::new();
    let join_error =
        |err: tokio::task::JoinError| std::io::Error::other(format!("bench op task: {err}"));
    let spawn_one = |joins: &mut JoinSet<(ObjectClient, WorkerTotals)>,
                     idle: &mut Vec<ObjectClient>,
                     op_index: usize,
                     measured: bool| {
        let mut client = idle.pop().expect("an idle bench client per free slot");
        let config = Arc::clone(&config);
        let run_prefix = Arc::clone(&run_prefix);
        let prefill_keys = Arc::clone(&prefill_keys);
        let payload = Arc::clone(&payload);
        let live_stats = measured.then(|| Arc::clone(&live_stats));
        joins.spawn(async move {
            let mut delta = WorkerTotals::default();
            run_one_op(
                &mut client,
                &config,
                &run_prefix,
    ...
    match config.run_mode {
        ObjectBenchmarkRunMode::FixedOps { ops_per_worker } => {
            while op_index < ops_per_worker || !joins.is_empty() {
                while op_index < ops_per_worker && !idle.is_empty() {
                    spawn_one(&mut joins, &mut idle, op_index, true);
                    op_index += 1;
                }
                let Some(joined) = joins.join_next().await else {
                    break;
                };
                let (client, delta) = joined.map_err(join_error)?;
                idle.push(client);
                merge_bench_totals(&mut totals.reads, delta.reads);
                merge_bench_totals(&mut totals.writes, delta.writes);
            }
        }
        ObjectBenchmarkRunMode::Timed { warmup, duration } => {
            if !warmup.is_zero() {
                let warmup_deadline = Instant::now() + warmup;
                while Instant::now() < warmup_deadline {
                    while Instant::now() < warmup_deadline && !idle.is_empty() {
                        spawn_one(&mut joins, &mut idle, op_index, false);
                        op_index += 1;
                    }
                    let Some(joined) = joins.join_next().await else {
                        break;
                    };
                    let (client, _warmup_delta) = joined.map_err(join_error)?;
                    idle.push(client);
                }
                // Drain warmup stragglers so the barrier truly gates the
                // measured phase for every worker.
            ...
            let deadline = Instant::now() + duration;
            while Instant::now() < deadline {
                while Instant::now() < deadline && !idle.is_empty() {
                    spawn_one(&mut joins, &mut idle, op_index, true);
                    op_index += 1;
                }
                let Some(joined) = joins.join_next().await else {
            ...
            // Ops in flight at the deadline complete and count — the same
            // overshoot semantics as the sequential worker's final op.
            while let Some(joined) = joins.join_next().await {
                let (client, delta) = joined.map_err(join_error)?;
                idle.push(client);
                merge_bench_totals(&mut totals.reads, delta.reads);
...
fn record_success(
    ...
    totals.phases.kms_begin_us += phases.kms_begin.as_micros();
    totals.phases.kms_cluster_config_us += phases.kms_cluster_config.as_micros();
    ...
    totals.phases.target_write_wall_us += phases.target_write_wall.as_micros();
...
fn merge_bench_totals(into: &mut BenchTotals, from: BenchTotals) {
    ...
    into.phases.kms_begin_us += from.phases.kms_begin_us;
    into.phases.kms_cluster_config_us += from.phases.kms_cluster_config_us;
    ...
    into.phases.target_write_wall_us += from.phases.target_write_wall_us;
...
fn average_phases(totals: &PhaseTotals, ops: usize) -> ObjectPhaseTimes {
    ...
    ObjectPhaseTimes {
        ...
        kms_begin: Duration::from_micros((totals.kms_begin_us / ops) as u64),
        kms_cluster_config: Duration::from_micros((totals.kms_cluster_config_us / ops) as u64),
        ...
        target_write_wall: Duration::from_micros((totals.target_write_wall_us / ops) as u64),
