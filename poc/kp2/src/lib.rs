...
/// Sentinel `slot_index` meaning "the target allocates the granule itself". A write
/// carrying this in place of a concrete granule asks the storage target to choose a
/// free granule (its own allocator picks one), rather than landing at a granule the
/// client reserved centrally. The reply reports the chosen granule in
/// [`PackedWriteReplyEntry::location`] (`slot_index`) / the direct write's
/// `x-kst-granule-index` header, so the writer learns where the fragment landed.
/// Reusing the max sentinel keeps the wire layout unchanged (no `WRITE_ENTRY_BYTES`
/// bump): a real granule never reaches `u64::MAX`.
pub const GRANULE_ALLOCATE: u64 = u64::MAX;
