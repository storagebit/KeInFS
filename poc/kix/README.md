...
- clean replay, `20k` checkpoint entries + `512` delta frames:
  - `checkpoint-charlie.example`: about `27.1 ms p50`
  - `replay-roger.example`: about `80.6 ms p50`
...
- rebuild-from-media on deterministic chunk-media spans:
  - `checkpoint-charlie.example`, `4,096` live entries + `64` delta batches over `8,192` slots: about `2.15 s`
  - `replay-roger.example`, `8,192` live entries + `128` delta batches over `16,384` slots: about `8.16 s`
