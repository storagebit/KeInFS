...
#[test]
fn occupied_granules_lists_sorted_and_clear_frees_the_slot() {
    let dir = TestDir::new("occupied-granules");
    let config = test_config(dir.path(), 2, 1);
    let engine = KixEngine::open(config).unwrap();
    let client = engine.client();
    let a = ChunkId::from_seed(20);
    let b = ChunkId::from_seed(21);
    // gen 1 at slot 3, gen 2 at slot 1 (inserted out of slot order)
    client
        .upsert(a, 3, LocationRecord::extent(0, 4096, 65536, 65536, 1, 1))
        .unwrap();
    client
        .upsert(b, 1, LocationRecord::extent(0, 4096, 65536, 65536, 2, 2))
        .unwrap();
    // The GC scan sees every occupied granule, sorted by slot for stable paging.
    assert_eq!(client.occupied_granules(0), vec![(1, b, 2), (3, a, 1)]);
    // A drive with nothing occupied yields an empty list (not a panic).
    assert!(client.occupied_granules(1).is_empty());
    // Clearing the inverse entry frees the slot for the allocator AND drops it from the
    // scan — the GC free relies on this so a reclaimed granule becomes reusable.
    client.clear_granule_inverse(0, 3);
    assert_eq!(client.lookup_granule_chunk(0, 3), None);
    assert_eq!(client.occupied_granules(0), vec![(1, b, 2)]);
}
