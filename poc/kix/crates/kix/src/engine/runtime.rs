...
impl DriveArenaSet {
    ...
    pub(super) fn append_delta(&self, drive_id: u16, delta: DeltaEntry) -> Result<(), KixError> {
        ...
        // Capacity 1, not a rendezvous (0): the durability semantics are unchanged (this
        // caller still blocks on `resp_rx.recv()` until its delta batch is durable), but the
        // drive worker's reply (`resp.send`) no longer BLOCKS until this thread happens to be
        // scheduled to receive. With a rendezvous channel the worker serialized on N
        // context-switches per batch — one blocking handoff per reply — which (with fdatasync
        // ~1us here) was the actual write ceiling, not the io. A 1-slot buffer lets the worker
        // fire all replies and immediately start the next batch; each writer wakes in parallel.
