...
impl ChunkMediaHandle {
    ...
    /// Reads the self-describing object identity stamped in a live chunk's on-media
    /// header. Used by the lease-fenced GC to map an occupied granule back to the object
    /// fragment it holds (object_id/version/stripe/frag) so the reverse log + lease can be
    /// consulted. Validates the header against `chunk_id` + `record` like a read does.
    pub fn read_identity(
        &self,
        chunk_id: ChunkId,
        record: LocationRecord,
    ) -> io::Result<ChunkSelfDescribingIdentity> {
        let header = read_validated_live_header(&self.file, &self.config.span, chunk_id, record)?;
        Ok(ChunkSelfDescribingIdentity {
...
fn write_chunk_media_payload_with_file(
    ...
    let slot_len = CHUNK_MEDIA_SLOT_HEADER_BYTES
        .checked_add(record.stored_length as u64)
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "chunk-media slot overflow"))?;
    // Write only the header block plus the payload, rounded up to the O_DIRECT
    // block — never the slot's full capacity. The header records logical_length
    // and the checksum covers only the payload, so bytes beyond the aligned
    // payload span are never read back as data; zero-filling the whole slot on
    // every write multiplied physical IO by capacity/payload (measured 128x at
    // 16 MiB extents with 128 KiB fragments — the drives saturated on padding
    // and capped the cluster's small-fragment request rate).
    let payload_span = CHUNK_MEDIA_SLOT_HEADER_BYTES
        .checked_add(payload.len() as u64)
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "chunk-media slot overflow"))?;
    let align = CHUNK_MEDIA_ALIGN_BYTES as u64;
    let write_len = payload_span.div_ceil(align).saturating_mul(align).min(slot_len);
    let mut block = AlignedIoBuffer::zeroed(write_len as usize)?;
    let header = ChunkMediaSlotHeader {
        state: ChunkMediaEntryState::Live,
        drive_id: record.drive_id,
        chunk_id,
    ...
    let payload_dst = &mut block.as_mut_slice()[CHUNK_MEDIA_SLOT_HEADER_BYTES as usize
        ..CHUNK_MEDIA_SLOT_HEADER_BYTES as usize + payload.len()];
...
fn read_chunk_media_payload_with_file(
    ...
    // Read only the payload's aligned span, not the slot's full capacity — the
    // write side stores exactly this much, and a capacity-sized read multiplied
    // physical IO by capacity/payload for small fragments in large slots.
    let logical_len = (record.logical_length as usize).min(record.stored_length as usize);
    let align = CHUNK_MEDIA_ALIGN_BYTES;
    let read_len = logical_len
        .div_ceil(align)
        .saturating_mul(align)
        .clamp(align, record.stored_length.max(align as u32) as usize);
    let mut payload = AlignedIoBuffer::zeroed(read_len)?;
...
mod tests {
    ...
    fn deferred_lane_writes_share_a_single_barrier_and_roundtrip() {
        ...
        let written_a = handle
            .write_payload_to_lane_unsynced(0, 0, chunk_a, 1, ChunkSelfDescribingIdentity::default(), &payload_a)
            .unwrap();
        let written_b = handle
            .write_payload_to_lane_unsynced(1, 0, chunk_b, 1, ChunkSelfDescribingIdentity::default(), &payload_b)
            .unwrap();
        ...
        let written_a2 = handle
            .write_payload_to_lane_unsynced(0, next_lane, chunk_a, 2, ChunkSelfDescribingIdentity::default(), &payload_a2)
            .unwrap();
        handle.fdatasync().unwrap();
        assert_ne!(
