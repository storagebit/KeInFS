...
impl KixClient {
    ...
    /// Enumerates the occupied granules of a drive as `(slot, chunk_id, generation)`,
    /// sorted by slot so a caller can page through them with a stable cursor. The
    /// lease-fenced GC reads this to find candidate orphans (a granule the inverse shows
    /// occupied; whether it is committed is decided against the FDB reverse log).
    pub fn occupied_granules(&self, drive_id: u16) -> Vec<(u64, ChunkId, u32)> {
        let Some(inverse) = self.granule_inverse.get(&drive_id) else {
            return Vec::new();
        };
        let mut granules: Vec<(u64, ChunkId, u32)> = inverse
            .iter()
            .map(|entry| (*entry.key(), entry.value().chunk_id, entry.value().generation))
            .collect();
        granules.sort_unstable_by_key(|(slot, _, _)| *slot);
        granules
    }

    /// Removes a (drive, slot) entry from the granule inverse so the target-local
    /// allocator can reuse the slot. The forward-map `delete` retires the chunk but does
    /// NOT touch the inverse, so without this a freed granule would read as occupied
    /// forever. Generation-fenced by the caller (it verifies the live owner first).
    pub fn clear_granule_inverse(&self, drive_id: u16, slot_index: u64) {
        if let Some(inverse) = self.granule_inverse.get(&drive_id) {
            inverse.remove(&slot_index);
        }
    }
