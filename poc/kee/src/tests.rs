...
#[test]
fn accepts_general_geometry_rejects_over_galois_field() {
    // Generalized: arbitrary (data, parity) is valid as long as the total shard count
    // fits GF(2^8). 4+2 is fine now; only exceeding 256 total shards is rejected.
    EcProfile {
        data_fragments: 4,
        parity_fragments: 2,
        ..profile()
    }
    .validate_single_stripe()
    .unwrap();
    assert!(EcProfile {
        data_fragments: 200,
...
#[test]
fn replicated_1_plus_2_roundtrips() {
    // The "replicated" small-object profile: 1 data + 2 parity = 3 full-size copies, so a
    // granule-sized object writes 3 granule-filling fragments (not 10 sub-granule ones),
    // and any single surviving fragment reconstructs it.
    let profile = EcProfile {
        data_fragments: 1,
        parity_fragments: 2,
    ...
    profile.validate_single_stripe().unwrap();
    let object: Vec<u8> = (0..profile.max_object_bytes())
        .map(|i| (i as u8).wrapping_mul(31))
        .collect();
    let encoded = encode(&profile, &object).unwrap();
    assert_eq!(encoded.len(), 3, "1 data + 2 parity");
    assert!(verify(&profile, &encoded).unwrap());
    // Lose 2 of 3 (the maximum tolerable) — a single survivor reconstructs the data.
    let mut fragments: Vec<Option<Vec<u8>>> = encoded.iter().cloned().map(Some).collect();
    fragments[0] = None;
    fragments[2] = None;
    let restored = reconstruct(&profile, &mut fragments).unwrap();
    assert_eq!(restored[0], object, "data fragment reconstructed from one parity");
}
