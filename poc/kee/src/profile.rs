...
impl EcProfile {
    ...
    pub fn validate_single_stripe(&self) -> Result<(), KeeError> {
        self.validate()?;
        // Reed-Solomon over GF(2^8): the total shard count must fit the field. This
        // admits any (data, parity) profile — 8+2 for the large-object bandwidth path,
        // and 1+2 ("replicated", three full-size copies) for the granule-sized / small
        // object path that writes 3 granule-filling fragments instead of 10 sub-granule
        // ones. The data plane (placement + write/read fan-out) is already (k,m)-generic.
        if self.data_fragments + self.parity_fragments > 256 {
            return Err(KeeError::InvalidProfile(
                "data_fragments + parity_fragments must not exceed 256 (GF(2^8) Reed-Solomon limit)"
                    .to_string(),
            ));
        }
