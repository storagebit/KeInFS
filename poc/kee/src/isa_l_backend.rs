...
mod imp {
    ...
    /// True when this backend's fixed-shape fast path matches the profile. The
    /// ISA-L tables and pointer arrays are sized for one stripe shape; every other
    /// profile — replicated small-object classes especially — takes the software
    /// codec, whose (k, m) is dynamic.
    pub fn plan_supports(profile: &EcProfile) -> bool {
        profile.data_fragments == DATA_FRAGMENTS && profile.parity_fragments == PARITY_FRAGMENTS
    }
...
mod imp {
    ...
    /// The software plan's (k, m) is dynamic: every valid profile is supported.
    pub fn plan_supports(_profile: &EcProfile) -> bool {
        true
    }
    ...
    impl PreparedBackendPlan {
        pub fn new(profile: &EcProfile) -> Result<Self, KeeError> {
            ...
            Ok(Self {
                profile: profile.clone(),
                codec: ReedSolomon::new(profile.data_fragments, profile.parity_fragments)?,
            })
        }
...
pub use imp::{encode, plan_supports, reconstruct, PreparedBackendPlan};
