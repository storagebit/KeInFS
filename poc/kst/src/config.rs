...
fn parse_serve_args(args: &[String]) -> Result<ServeConfig, Box<dyn Error>> {
    ...
    // Default every worker pool to interrupt (park-when-idle). Busy-poll is a latency
    // optimization that is only safe when total spinners <= physical cores, and one KST
    // process cannot know how many sibling targets co-locate on the box. On a 12-target
    // host the old busy defaults spawned ~28 hard-spinning threads/target (336 box-wide
    // on 128 cores) that starved the async h2 receive pump. Operators on dedicated
    // single-target hosts can opt back into busy-poll per pool via the --*-mode flags.
    let mut lookup_mode_name = ModeName::Interrupt;
    ...
    let mut direct_read_mode_name = ModeName::Interrupt;
