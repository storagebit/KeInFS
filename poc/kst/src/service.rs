...
pub(crate) struct TargetRouter {
    ...
    // Target-local granule allocator: a write that carries `kp2::GRANULE_ALLOCATE` in
    // place of a concrete granule has this drive choose a free one, so the write needs
    // no centrally reserved granule.
...
impl TargetRouter {
    ...
    pub(crate) fn route_buffered(
        ...
        match (method, path) {
            ...
            (Method::GET, "/v1/granules/occupied") => {
                self.handle_occupied_granules(uri.query(), body)
            }
            ...
            (method, _) => Err(ServiceError::new(
                ...
                format!(
                    "KST does not expose {} {}; use /v1/info, /v1/stats, /v1/granules/occupied, /v1/chunk/<chunk-id>, /v1/kp2/chunk-pack, or /v1/kp2/chunk-pack/read",
                    method, uri
                ),
                true,
            )),
    ...
    /// Lists the drive's occupied granules with their on-media object identity, for the
    /// lease-fenced GC to find candidate orphans. Pages by slot: `cursor` is an exclusive
    /// lower bound (resume after it) and `limit` bounds the granules examined per call
    /// (bounding the on-media identity reads). The reply's `next_cursor` is the resume
    /// point, or null when the drive has been fully walked.
    fn handle_occupied_granules(
        &self,
        query: Option<&str>,
        body: Vec<u8>,
    ) -> Result<ServiceResponse, ServiceError> {
        if !body.is_empty() {
            return Err(ServiceError::new(
                StatusCode::BAD_REQUEST,
                "KST occupied-granule requests must not include a request body".to_string(),
                true,
            ));
        }
        const DEFAULT_OCCUPIED_LIMIT: usize = 256;
        const MAX_OCCUPIED_LIMIT: usize = 4096;
        let cursor = parse_query_u64_present(query, "cursor");
        let limit = parse_query_u64_present(query, "limit")
            .map(|value| value as usize)
            .unwrap_or(DEFAULT_OCCUPIED_LIMIT)
            .clamp(1, MAX_OCCUPIED_LIMIT);
        let drive_id = self.media.config().drive_id;
        let occupied = self.client.occupied_granules(drive_id);
        let mut entries = Vec::new();
        let mut examined = 0usize;
        let mut next_cursor: Option<u64> = None;
        // `occupied` is sorted by slot, so resume = last slot examined.
        let mut last_examined = cursor.unwrap_or(0);
        for (slot, chunk_id, generation) in occupied {
            if cursor.is_some_and(|c| slot <= c) {
                continue;
            }
            if examined >= limit {
                next_cursor = Some(last_examined);
                break;
            }
            examined += 1;
            last_examined = slot;
            // The chunk may have been deleted between the inverse snapshot and now, or
            // its on-media header may be unreadable; either way it cannot be a safe
            // reclaim candidate, so skip it (paging still advances past its slot).
            let Some(record) = self.client.get(chunk_id).ok().flatten() else {
                continue;
            };
            let Ok(identity) = self.media.read_identity(chunk_id, record) else {
                continue;
            };
            entries.push(OccupiedGranuleDocument {
                slot,
                chunk_id: hex::encode(chunk_id.0),
                generation,
                object_id: identity.object_id,
                object_version: identity.object_version as u32,
                stripe: identity.stripe as u32,
                frag: identity.frag as u32,
            });
        }
        let payload = encode_json(&OccupiedGranulesPage {
            entries,
            next_cursor,
        })
        .map_err(json_error)?;
    ...
    fn handle_chunk(
        ...
        match method {
            ...
            Method::DELETE => self.handle_delete(chunk_id, uri.query(), body),
    ...
    /// Picks a free granule on this drive for an allocate-mode write. The closure reports
    /// whether a slot already holds a durably published chunk (the KIX inverse is the
    /// truth), so the allocator never hands out an occupied granule. Returns the chosen
    /// slot marked pending; the caller MUST `release` it on every exit (rollback or
    /// publish success). `None` only when the drive is full.
    fn allocate_granule(&self) -> Option<u64> {
        let drive_id = self.media.config().drive_id;
        self.granule_allocator
            .allocate(|slot| self.client.lookup_granule_chunk(drive_id, slot).is_some())
    }
    ...
    fn write_chunk_with_payload(
        ...
        // Allocate-mode: the client asked the target to choose the granule. Pick a free
        // one and drop the pending marker on every exit below (the KIX inverse protects
        // it once published). A non-sentinel slot is the client's centrally reserved
        // granule and is used verbatim (and never marked pending, so releasing it is a
        // harmless no-op).
        let allocate_mode = slot_index == kp2::GRANULE_ALLOCATE;
        let slot_index = if allocate_mode {
            self.allocate_granule().ok_or_else(|| {
                ServiceError::new(
                    StatusCode::INSUFFICIENT_STORAGE,
                    format!(
                        "KST has no free granule to allocate for chunk {:?}: the drive is full",
                        chunk_id
                    ),
                    true,
                )
        ...
        let result =
            self.write_chunk_to_slot(chunk_id, slot_index, generation, identity, body);
        if allocate_mode {
            // The granule is now either published (KIX inverse protects it) or rolled
            // back; either way the pending reservation is no longer needed.
            self.granule_allocator.release(slot_index);
        }
        result
    }

    fn write_chunk_to_slot(
        &self,
        chunk_id: ChunkId,
        slot_index: u64,
        generation: u32,
        ...
        // committed owner fully intact. See committed-occupancy design.
    ...
    fn handle_kp2_write(
        ...
        let mut pack = decode_write_request(&body).map_err(kp2_request_error)?;
        ...
        // Allocate-mode resolution (before staging + the dedup check): each entry that
        // carries `kp2::GRANULE_ALLOCATE` has this drive pick a distinct free granule.
        // The allocator marks each pending, so two entries never resolve to the same
        // slot (the dedup guard below then holds). Released after publish (or on any
        // post-decode error) — published slots are protected by the KIX inverse, failed
        // ones return to the free pool.
        let mut allocated_slots: Vec<u64> = Vec::new();
        for entry in &mut pack.entries {
            if entry.slot_index == kp2::GRANULE_ALLOCATE {
                match self.allocate_granule() {
                    Some(slot) => {
                    ...
                    None => {
                        for slot in &allocated_slots {
                            self.granule_allocator.release(*slot);
                        }
                        return Err(ServiceError::new(
                            StatusCode::INSUFFICIENT_STORAGE,
                            "KST has no free granule to allocate for a packed write entry: the drive is full"
                                .to_string(),
                            true,
                        ));
                    }
        ...
        let release_allocated = |router: &Self| {
            for slot in &allocated_slots {
                router.granule_allocator.release(*slot);
            }
        };
        ...
        let staged = match self.stage_packed_writes(
            pack.entries,
            &mut media_prepare,
            &mut media_write_io,
            &mut write_lookup,
        ...
        for stage in staged {
            match stage {
                ...
                PackEntryStage::Written {
                    ...
                    // See committed-occupancy design.
                    if let Some(owner) = reservation.current {
                        if owner.chunk_id != chunk_id {
                            slot_publication.rollback();
                            self.stats.record_committed_slot_write_rejection();
                    ...
                    match self.client.upsert(chunk_id, slot_index, record) {
                        Ok(()) => {
                            ...
                            kix_publish += publish_cost;
                            // Do NOT use `?` here: an early return between allocate and the
                            // end-of-request release would leak this pack's allocate-mode
                            // pending markers (shrinking allocatable space until restart).
                            // Report the (rare, poisoned-lock) commit failure per entry and
                            // continue so the release sweep still runs.
                            if let Err(err) =
                                slot_publication.commit(PublishedSlotOwner { chunk_id, record })
                            {
                                entries.push(PackedWriteReplyEntry {
                                    ...
                                    location: None,
                                    error: Some(format!(
                                        "KST published chunk {:?} into KIX but could not commit the slot publication: {}",
        ...
        // Every staged entry has reached a terminal state (published into the KIX
        // inverse, or rolled back). Drop the pending markers for the granules this
        // request allocated: published ones are now protected by the inverse, and
        // rolled-back ones return to the free pool.
        release_allocated(self);
        self.stats.record_phase(
            RpcKind::Write,
            RequestPhase::MediaWritePrepare,
    ...
    fn handle_delete(
        ...
        // Generation-fenced free (lease-fenced GC): when the caller supplies the
        // `generation` it observed, the delete proceeds only if the durable granule
        // inverse still maps the slot to exactly this (chunk_id, generation). This makes
        // the physical free a compare-and-delete against the live stamp, so a stale GC
        // authorization cannot free a slot a newer write now owns (and closes the
        // post-restart window where the inverse is still repopulating).
        let expected_generation = parse_query_u32_present(query, "generation");
        ...
        if let Some(expected_generation) = expected_generation {
            // The granule must still hold exactly the (chunk_id, generation) the caller
            // observed; otherwise it was overwritten or already freed — refuse, reporting
            // "not deleted" so the GC treats it as a no-op (the granule it meant to free is
            // gone or is now someone else's).
            match self.client.lookup_granule_chunk(drive_id, slot_index) {
                Some((owner_chunk, owner_generation))
                    if owner_chunk == chunk_id && owner_generation == expected_generation => {}
                _ => {
                    let payload = encode_json(&DeleteChunkDocument {
                        deleted: false,
                        tombstone_generation: 0,
        ...
        if delete_result.is_ok() {
            // Free the granule for the target-local allocator: the forward-map delete
            // retires the chunk but leaves the (slot -> owner) inverse entry, which would
            // otherwise keep the slot marked occupied forever.
            self.client.clear_granule_inverse(drive_id, slot_index);
        }
...
/// One occupied granule reported to the lease-fenced GC: its slot, the chunk it holds
/// (id + generation, for the generation-fenced free), and the on-media object identity
/// the GC maps to the reverse-log row + write lease.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub(crate) struct OccupiedGranuleDocument {
    pub slot: u64,
    pub chunk_id: String,
    pub generation: u32,
    pub object_id: u32,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub(crate) struct OccupiedGranulesPage {
    pub entries: Vec<OccupiedGranuleDocument>,
...
/// Parses an optional u32 query parameter, distinguishing "absent" (None) from a present
/// value — needed where 0 is a meaningful value (e.g. a fresh write's generation).
fn parse_query_u32_present(query: Option<&str>, key: &str) -> Option<u32> {
    parse_query_value(query, key)
        .ok()
        .and_then(|value| value.parse::<u32>().ok())
}

/// Parses an optional u64 query parameter (e.g. a paging cursor); None when absent.
fn parse_query_u64_present(query: Option<&str>, key: &str) -> Option<u64> {
    parse_query_value(query, key)
        .ok()
