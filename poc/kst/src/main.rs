...
/// Default async-runtime worker-thread count. The tokio default is `num_cpus`, which is
/// catastrophic when many KSTs co-locate on one large (e.g. 256-core) box: one runtime
/// per process x num_cpus = thousands of threads oversubscribing the NUMA-pinned cores,
/// thrashing the scheduler and starving the h2 connection pumps that deliver each
/// request's body frames — the dominant per-frame receive latency. The async runtime only
/// drives socket IO + light routing (the media path runs on dedicated execution workers),
/// so a small bound is ample. Override with KST_TOKIO_WORKER_THREADS for the per-KST core
/// budget (e.g. a single-KST box can raise it).
const DEFAULT_TOKIO_WORKER_THREADS: usize = 8;

fn main() -> Result<(), Box<dyn Error>> {
    let worker_threads = std::env::var("KST_TOKIO_WORKER_THREADS")
        .ok()
