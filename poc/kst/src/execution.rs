...
impl DirectExecutionHandle {
    ...
    ) -> Result<oneshot::Receiver<DirectExecutionOutcome>, DirectExecutionSubmitError> {
...
/// What a direct-execution worker hands back to the request task: the response plus the two
/// worker-side phase durations the request task cannot see (it runs on a different thread).
/// Returning them here lets the request handler assemble one correlated per-fragment trace
/// line instead of relying on the per-RPC-type phase aggregates, which average these across
/// every concurrent fragment.
pub(crate) struct DirectExecutionOutcome {
    pub(crate) result: Result<ServiceResponse, ServiceError>,
    /// Time the task spent queued between `submit` and worker pickup (worker saturation).
    pub(crate) queue_wait: Duration,
    /// Time the worker spent in `handle_direct_chunk_*` — media write + fsync + KIX publish
    /// for a write. Its sub-phases stay broken out in the per-RPC phase stats.
    pub(crate) route_execute: Duration,
}
...
fn direct_execution_worker_loop(
    ...
    while let Some(task) = next_task(&receiver, mode) {
        ...
        router
            .stats
            .record_phase(task.rpc, RequestPhase::ExecutionQueueWait, queue_wait);
        let (result, route_execute) = match task.request {
            DirectExecutionRequest::Read { chunk_id } => {
                let started = Instant::now();
                let result = router.handle_direct_chunk_read(chunk_id);
                let route_execute = started.elapsed();
                router
                    .stats
                    .record_phase(task.rpc, RequestPhase::RouteExecute, route_execute);
        ...
        let _ = task.completion.send(DirectExecutionOutcome {
            result,
            queue_wait,
            route_execute,
        });
...
struct DirectExecutionTask {
    ...
    enqueued_at: Instant,
    request: DirectExecutionRequest,
    completion: oneshot::Sender<DirectExecutionOutcome>,
}
