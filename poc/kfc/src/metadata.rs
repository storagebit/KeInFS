...
impl MetadataClient {
    pub(crate) async fn connect(kms_endpoints: &[String]) -> Result<Self, DynError> {
    ...
    pub(crate) async fn get_bucket(&self, bucket_id: &str) -> Result<BucketRecord, DynError> {
    ...
    pub(crate) async fn bucket_entry_id(&self, bucket_id: &str) -> Result<String, DynError> {
    ...
    pub(crate) async fn list_children_all(
        &self,
        namespace_id: &str,
        parent_entry_id: &str,
        limit: u32,
    ...
    pub(crate) async fn resolve_object_size(
        &self,
        bucket_id: &str,
        key: &str,
    ) -> Result<u64, DynError> {
        ...
        let reply = rpc_timeout(
            kms.resolve_object_read(Request::new(ResolveObjectReadRequest {
                bucket_id: bucket_id.to_string(),
                key: key.to_string(),
            })),
        )
